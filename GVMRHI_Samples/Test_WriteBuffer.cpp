...
int main()
{
    ...
    eastl::vector<uint32_t> bufferData = eastl::vector<uint32_t>(screen_width * screen_height, uint32_t(255) << 0);
    mainQueue->writeBuffer(GVM::RHI::BufferRange(buffer), bufferData.data(), bufferData.size());
    ...
    mainQueue->writeTexture({.texture = texture, .origin = {0, 0, 0}, .mipLevel = 0, .aspect = GVM::RHI::TextureAspect::All}, bufferData.data(), bufferData.size() * sizeof(bufferData[0]), {}, {screen_width, screen_height, 1});
