#include "WebgpuSkinningRuntimeAdapter.hpp"
...
namespace GVM::ThreeSamples
{
    namespace
    {
        ...
        struct WebgpuSkinningTextureUpload final
        {
        ...
        float webgpuSkinningSrgbToLinear(float value)
        {
        ...
        glm::mat4 composeWebgpuSkinningNode(const MichelleGlbNode &node)
        {
        ...
        glm::mat4 computeWebgpuSkinningRestWorld(
            ...
            uint32_t nodeIndex)
        {
            ...
            {
                world *= composeWebgpuSkinningNode(asset.nodes[*iterator]);
            }
            return world;
        }

        /** Builds one explicit material mip upload without automatic GPU generation. */
        WebgpuSkinningTextureUpload buildWebgpuSkinningTextureUpload(
            const RgbaImageData &baseImage,
            bool srgb)
        {
            WebgpuSkinningTextureUpload upload;
        ...
        void appendWebgpuSkinningPayload(
        ...
        void updateWebgpuSkinningVertices(
            const MichelleGlbAsset &asset,
            const eastl::vector<glm::vec4> &positions,
            const eastl::vector<glm::vec4> &normals,
            const glm::mat4 &model,
            eastl::vector<WebgpuSkinningHostVertex> &vertices)
        {
        ...
        void prepareWebgpuSkinningEvidencePath(
        ...
        void writeWebgpuSkinningEvidence(
            const eastl::string &path,
            const std::string &text)
        {
            if (path.empty()) return;
            const std::filesystem::path outputPath(path.c_str());
            prepareWebgpuSkinningEvidencePath(outputPath);
            std::ofstream output(outputPath, std::ios::trunc);
            output << text;
            if (!output)
            {
                throw std::runtime_error(
                    "Could not write webgpu_skinning evidence.");
            }
        }
    } // namespace

    void WebgpuSkinningRuntimeAdapter::initializeResources(
        ...
        const ThreeSampleHostOptions &options)
    {
        ...
        if (options.caseId != "webgpu_skinning" || !validScenario ||
            options.width != 800u || options.height != 500u ||
            options.randomSeed != DefaultThreeRandomSeed ||
            options.assetRoot.empty() || !options.inputReplayPath.empty())
        {
            throw std::invalid_argument(
                "webgpu_skinning requires its three frozen r185 scenarios.");
        }
        device = inDevice;
        ...
        objectModel = computeWebgpuSkinningRestWorld(
        ...
        materialData.ambientColorAndPointIntensity = glm::vec4(
            webgpuSkinningSrgbToLinear(0x44u / 255.0f),
            webgpuSkinningSrgbToLinear(0x66u / 255.0f),
            1.0f,
        ...
        eastl::array<WebgpuSkinningTextureUpload, 4u> textureUploads;
        textureUploads[0u] = buildWebgpuSkinningTextureUpload(
            asset.textures[0u], true);
        textureUploads[1u] = buildWebgpuSkinningTextureUpload(
            asset.textures[1u], false);
        textureUploads[2u] = buildWebgpuSkinningTextureUpload(
            asset.textures[2u], true);
        textureUploads[3u] = buildWebgpuSkinningTextureUpload(
        ...
        {
            throw std::runtime_error(
                "Could not create the webgpu_skinning Set encoder.");
        }
        GVM::Core::RenderSetAllocInfo allocation;
        ...
        appendWebgpuSkinningPayload(
            allocation, WebgpuSkinningSceneRenderSetComponents::vertices,
            "WebgpuSkinningVertices", vertices.data(),
            vertices.size() * sizeof(WebgpuSkinningHostVertex), 1u);
        appendWebgpuSkinningPayload(
            allocation, WebgpuSkinningSceneRenderSetComponents::indices,
            "WebgpuSkinningIndices", asset.indices.data(),
            asset.indices.size() * sizeof(uint32_t), 1u);
        appendWebgpuSkinningPayload(
            allocation, WebgpuSkinningSceneRenderSetComponents::objects,
            "WebgpuSkinningObject", &objectData, sizeof(objectData), 1u);
        appendWebgpuSkinningPayload(
            allocation, WebgpuSkinningSceneRenderSetComponents::instances,
            "WebgpuSkinningInstance", &instanceData, sizeof(instanceData), 1u);
        appendWebgpuSkinningPayload(
            allocation, WebgpuSkinningSceneRenderSetComponents::materials,
            "WebgpuSkinningMaterial", &materialData, sizeof(materialData), 1u);
        GVM::Core::RenderSetTextureComponentAllocInfo textureInfo;
        textureInfo.textureComponentHandle =
            WebgpuSkinningSceneRenderSetComponents::textures;
        ...
        {
            const WebgpuSkinningTextureUpload &upload = textureUploads[texture];
    ...
    void WebgpuSkinningRuntimeAdapter::beforeFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex)
    {
        ...
        {
            throw std::runtime_error(
                "Could not update the webgpu_skinning vertex component.");
        }
        encoder->setBufferComponentData(
            entityIndex,
            WebgpuSkinningSceneRenderSetComponents::vertices,
            vertices.data(), vertices.size() * sizeof(WebgpuSkinningHostVertex),
            0u, static_cast<uint32_t>(vertices.size()));
        renderer.executeRenderSetCommand(SceneRenderSetHandle, encoder);
    }

    void WebgpuSkinningRuntimeAdapter::afterFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex,
        ...
        uint32_t height)
    {
        ...
        {
            throw std::overflow_error("webgpu_skinning capture is too large.");
        }
        ...
        if (!options.captureRgbaPath.empty())
        {
            ...
            prepareWebgpuSkinningEvidencePath(path);
            ...
            {
                throw std::runtime_error(
                    "Could not write webgpu_skinning RGBA.");
            }
        }
        std::ostringstream metadata;
        metadata << "{\n  \"schemaVersion\":1,\n"
                 << "  \"caseId\":\"webgpu_skinning\",\n"
        ...
        scene << "{\n  \"schemaVersion\":1,\n"
              ...
              << "  \"scenePassCount\":2,\n"
              << "  \"screenPassCount\":1,\n"
              << "  \"screenPasses\":[\"procedural-gradient-background\"],\n"
              << "  \"scenePassSequence\":[\n"
              ...
              << "    \"renderSetCount\":1,\n"
              << "    \"renderSetId\":\"scene\",\n"
              << "    \"renderSetType\":\"WebgpuSkinningSceneRenderSet\",\n"
              << "    \"renderableObjectCount\":1,\n"
              << "    \"entityCount\":1,\n"
              ...
              << "    \"directDrawFallback\":false,\n"
              << "    \"scenePasses\":[\n"
              << "      {\"name\":\"main-michelle-double-back\",\"renderClass\":\"WebgpuSkinningBackPass\",\"renderSetId\":\"scene\",\"renderSetBindingCount\":1,\"drawMode\":\"render-set-indexed-indirect\",\"invocationCount\":1,\"drawCommandCount\":1,\"usesStandaloneGeometry\":false,\"usesExplicitDrawCount\":false},\n"
              << "      {\"name\":\"main-michelle-front\",\"renderClass\":\"WebgpuSkinningFrontPass\",\"renderSetId\":\"scene\",\"renderSetBindingCount\":1,\"drawMode\":\"render-set-indexed-indirect\",\"invocationCount\":1,\"drawCommandCount\":1,\"usesStandaloneGeometry\":false,\"usesExplicitDrawCount\":false}\n"
              << "    ]\n"
              << "  }]\n}\n";
        writeWebgpuSkinningEvidence(options.sceneSnapshotPath, scene.str());
    ...
    void WebgpuSkinningRuntimeAdapter::shutdown(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options)
    {
        (void)renderer;
