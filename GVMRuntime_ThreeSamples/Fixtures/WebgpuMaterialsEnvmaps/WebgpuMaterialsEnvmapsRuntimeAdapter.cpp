#include "WebgpuMaterialsEnvmapsRuntimeAdapter.hpp"
...
namespace GVM::ThreeSamples
{
    namespace
    {
        ...
        constexpr const char *EnvironmentEquirectangularReplaySha256 =
            "0b6364c22e0179afa2161f87e5b417c29bae34f573a8c6b01adbca41c60939e1";
    ...
    void WebgpuMaterialsEnvmapsRuntimeAdapter::initializeResources(
        GVM::Core::DeviceProxy inDevice,
        const ThreeSampleHostOptions &options)
    {
        const bool initial =
            options.scenarioId == "initial-cube" &&
            options.targetFrame == 0u;
        const bool canonical =
            options.scenarioId == "canonical-assets" &&
            options.targetFrame == 0u;
        equirectangularScenario =
            options.scenarioId == "equirectangular-refraction" &&
            options.targetFrame == 1u;
        rotationScenario =
            options.scenarioId == "rotated-synchronized" &&
            options.targetFrame == 60u;
        const bool replayScenario =
            equirectangularScenario || rotationScenario;
        if (options.caseId != "webgpu_materials_envmaps" ||
            (!initial && !canonical && !replayScenario) ||
        ...
        const float phi = EnvironmentPi * 0.5f;
        ...
        cameraPositionAndMode =
            glm::vec4(
                cameraPosition,
                equirectangularScenario ? 1.0f : 0.0f);
        ...
        rotationRows = buildEnvironmentRotationRows(
            rotationScenario ? 0.061f : 0.0f);
    }

    void WebgpuMaterialsEnvmapsRuntimeAdapter::beforeFrame(
    ...
    void WebgpuMaterialsEnvmapsRuntimeAdapter::afterFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex,
    ...
    void WebgpuMaterialsEnvmapsRuntimeAdapter::writeArtifacts(
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex,
        uint32_t width,
        uint32_t height,
        const eastl::vector<uint8_t> &rgba) const
    {
        ...
        if (!options.sceneSnapshotPath.empty())
        {
            ...
            output
                ...
                << "\",\n"
                << "  \"refraction\":"
                << (equirectangularScenario ? "true" : "false") << ",\n"
                << "  \"backgroundMaterialRotationSynchronized\":"
                << (rotationScenario ? "true" : "false") << ",\n"
            ...
            options.scenarioId == "canonical-assets")
        {
            ...
            output
                ...
                << "  \"canonicalState\":\"six-cube-faces-one-equirectangular-image\",\n"
                << "  \"result\":{\"renderableObjectCount\":0,"
                << "\"sceneRootCount\":1,"
                << "\"canonicalSceneSha256\":"
                << "\"9ebbcd8b577058febefce8c03bddd9d3cfbb9f6e63116fe47443cd83ccf3b136\","
                << "\"cubeFaceCount\":6,\"cubeFaceExtent\":[1024,1024],"
                << "\"equirectangularExtent\":[4096,2048],"
                << "\"sphereVertexCount\":15360,"
                << "\"sphereIndexCount\":15360}\n}\n";
        }
    ...
    void WebgpuMaterialsEnvmapsRuntimeAdapter::shutdown(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options)
    {
        (void)renderer;
