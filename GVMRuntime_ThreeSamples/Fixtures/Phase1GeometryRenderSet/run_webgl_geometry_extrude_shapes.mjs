...
const caseId = 'webgl_geometry_extrude_shapes';
const shardName = 'Phase1GeometryRenderSet';
const replayRelativePath = path.join(
  'inputs', 'webgl_geometry_extrude_shapes_orbit.json');
const replaySourcePath = path.join(scriptDirectory, 'Inputs', path.basename(replayRelativePath));
const replaySha256 = '414a4afe4ed204bfcf3a17031b7f1b044f17f1d9814167dc902bfb49f06551d7';
...
/** Loads the dedicated extrusion example from the frozen Manifest. */
async function loadSelectedExample(manifestPath) {
  ...
  if (!Array.isArray(manifest.examples) || manifest.examples.length !== 588) {
    throw new Error('Extrusion fixture requires the frozen 588-item Three r185 Manifest.');
  }
  ...
  if (example?.status !== 'phase1_required'
    ...
    || example.renderSetType !== 'WebglGeometryExtrudeShapesSceneRenderSet'
...
/** Locks the canonical Trackball replay into the runner asset root. */
async function stageReplay(outputDirectory) {
  const bytes = await fs.readFile(replaySourcePath);
  const actualSha256 = createHash('sha256').update(bytes).digest('hex');
  if (actualSha256 !== replaySha256) {
    throw new Error(`${replaySourcePath}: Trackball replay identity differs.`);
  }
...
/** Runs every extrusion scene through the full strict matrix. */
...
/** Executes the strict dedicated extrusion fixture and writes summaries. */
async function main() {
  ...
  console.log(
    `Extrusion fixture ${status}: ${path.join(outputDirectory, 'summary.json')}`);
