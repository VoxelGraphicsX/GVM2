#include "WebgpuMultipleElementsRuntimeAdapter.hpp"
...
namespace GVM::ThreeSamples
{
    namespace
    {
        ...
        // the compatibility transform keeps the same phase across pipelines.
        constexpr double ReferenceEpochSeconds = 1700000000.0;
        constexpr uint32_t RandomPreamble = 200u;
        constexpr uint32_t RandomStridePerScene = 34u;
        constexpr char WebgpuMultipleElementsScrollReplaySha256[] =
            "863b4bea34c59991d42a455e5235f41df10ed54c9640cf6c61402ce0caa2a839";
        constexpr char WebgpuMultipleElementsOrbitReplaySha256[] =
            "309268bfa3e3e39e0a4a03553690460afe8ec662ec04749446df9286ac3f3352";
        ...
        void prepareWebgpuMultipleElementsPath(const eastl::string &pathValue)
        {
        ...
        eastl::string calculateWebgpuMultipleElementsReplaySha256(const eastl::string &pathValue)
        {
            std::ifstream input(pathValue.c_str(), std::ios::binary | std::ios::ate);
            if (!input) throw std::runtime_error("webgpu_multiple_elements could not open its input replay.");
            const std::streamsize size = input.tellg();
            if (size < 0 || size > static_cast<std::streamsize>(UINT32_MAX))
                throw std::runtime_error("webgpu_multiple_elements input replay is too large.");
            eastl::vector<uint8_t> bytes(static_cast<size_t>(size));
            input.seekg(0, std::ios::beg);
            if (size > 0 && !input.read(reinterpret_cast<char *>(bytes.data()), size))
                throw std::runtime_error("webgpu_multiple_elements could not read its input replay.");
        ...
        void appendWebgpuMultipleElementsBuffer(GVM::Core::RenderSetAllocInfo &allocation,
        ...
        void appendWebgpuMultipleElementsTriangle(WebgpuMultipleElementsEntity &entity,
                                       const glm::vec3 &a,
                                       const glm::vec3 &b,
                                       const glm::vec3 &c,
        ...
        float nextWebgpuMultipleElementsRandom(uint32_t &state)
        {
        ...
        glm::vec3 webgpuMultipleElementsHslToLinear(float hue)
        {
            const float q = 0.75f + 1.0f - 0.75f;
            const float p = 2.0f * 0.75f - q;
        ...
        void appendWebgpuMultipleElementsCardBackground(WebgpuMultipleElementsEntity &entity)
        {
            const uint32_t base = static_cast<uint32_t>(entity.vertices.size());
        ...
        void buildWebgpuMultipleElementsBox(WebgpuMultipleElementsEntity &entity)
        {
        ...
        void buildWebgpuMultipleElementsSphere(WebgpuMultipleElementsEntity &entity)
        {
            ...
            for (uint32_t ring = 0u; ring < Rings; ++ring)
            {
                ...
                for (uint32_t segment = 0u; segment < Segments; ++segment)
                {
                    ...
                    if (ring != 0u)
                        appendWebgpuMultipleElementsTriangle(entity, a, b, c, glm::normalize(a + b + c));
                    if (ring + 1u != Rings)
                        appendWebgpuMultipleElementsTriangle(entity, a, c, d, glm::normalize(a + c + d));
                }
            }
        }

        /** Generates the exact radius-.5 DodecahedronGeometry(0.5) triangles. */
        void buildWebgpuMultipleElementsDodecahedron(WebgpuMultipleElementsEntity &entity)
        {
            // Keep the polyhedron construction in double precision until the
            // final Float32 vertex upload, matching Three's JavaScript
            // PolyhedronGeometry normalization order.
            ...
            for (uint32_t triangle = 0u; triangle < sizeof(indexList) / sizeof(indexList[0]); triangle += 3u)
            {
                ...
                appendWebgpuMultipleElementsTriangle(entity, a, c, b,
                    glm::normalize(glm::cross(b - a, c - a)));
            }
        }

        /** Generates the flat-shaded CylinderGeometry(0.5,0.5,1,12) stream. */
        void buildWebgpuMultipleElementsCylinder(WebgpuMultipleElementsEntity &entity)
        {
            constexpr uint32_t Segments = 12u;
            constexpr float Radius = 0.5f;
            ...
            for (uint32_t segment = 0u; segment < Segments; ++segment)
            {
                ...
                appendWebgpuMultipleElementsTriangle(entity, bottom0, bottom1, top1, sideNormal);
                appendWebgpuMultipleElementsTriangle(entity, bottom0, top1, top0, sideNormal);
                appendWebgpuMultipleElementsTriangle(entity, top0, top1, glm::vec3(0.0f, HalfHeight, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
                appendWebgpuMultipleElementsTriangle(entity, bottom0, glm::vec3(0.0f, -HalfHeight, 0.0f), bottom1, glm::vec3(0.0f, -1.0f, 0.0f));
            }
        }

        /** Generates the eight-sided cone used by the upstream webgpuMultipleElements example. */
        void buildWebgpuMultipleElementsCone(WebgpuMultipleElementsEntity &entity)
        {
            ...
            for (uint32_t segment = 0u; segment < SegmentCount; ++segment)
            {
                ...
                appendWebgpuMultipleElementsTriangle(entity, apex, p0, p1,
                                           glm::normalize(glm::cross(p0 - apex, p1 - apex)));
                appendWebgpuMultipleElementsTriangle(entity, center, p1, p0, glm::vec3(0.0f, -1.0f, 0.0f));
            }
        }
                                    ...
                                    float radius)
        {
            for (uint32_t ring = 0u; ring < rings; ++ring)
            {
                ...
                for (uint32_t segment = 0u; segment < segments; ++segment)
                {
                    ...
                    const glm::vec3 c(radius * float(std::sin(p1) * std::cos(u1)), radius * float(std::cos(p1)), radius * float(std::sin(p1) * std::sin(u1)));
                    const glm::vec3 d(radius * float(std::sin(p0) * std::cos(u1)), radius * float(std::cos(p0)), radius * float(std::sin(p0) * std::sin(u1)));
                    appendWebgpuMultipleElementsTriangle(entity, a, b, c, glm::normalize(a));
                    appendWebgpuMultipleElementsTriangle(entity, a, c, d, glm::normalize(a));
                }
            }
        }

        /** Builds the Three perspective matrix with the generated-backend Y convention. */
        glm::mat4 webgpuMultipleElementsProjection(uint32_t width, uint32_t height)
        {
            const float aspect = float(width) / float(height);
            return glm::perspective(glm::radians(70.0f), aspect, 0.01f, 10.0f);
        }

        /** Validates the four deterministic target/webgpuMultipleElements scenarios. */
        /** Validates the frozen WebgpuMultipleElements scenario matrix and output contract. */
        void validateWebgpuMultipleElementsOptions(const ThreeSampleHostOptions &options)
        {
            ...
            if (options.caseId != "webgpu_multiple_elements"
                || (!scenario0 && !scenario1 && !scenario2 && !scenario3)
                || options.width != 800u || options.height != 500u
                || options.randomSeed != DefaultThreeRandomSeed)
            {
                throw std::invalid_argument("webgpu_multiple_elements scenario does not match the locked r185 contract.");
            }
            const bool replayScenario = scenario2 || scenario3;
            if (replayScenario != !options.inputReplayPath.empty())
                throw std::invalid_argument("webgpu_multiple_elements input replay is required only for scroll/orbit scenarios.");
            if (replayScenario)
            {
                const eastl::string actualHash = calculateWebgpuMultipleElementsReplaySha256(options.inputReplayPath);
                const eastl::string expectedHash = scenario2
                    ? WebgpuMultipleElementsScrollReplaySha256
                    : WebgpuMultipleElementsOrbitReplaySha256;
                if (actualHash != expectedHash)
                    throw std::invalid_argument("webgpu_multiple_elements input replay SHA-256 does not match the locked document.");
            }
        }
    }

    void WebgpuMultipleElementsRuntimeAdapter::initializeResources(
        GVM::Core::AbstractRendererImpl &renderer,
        GVM::Core::DeviceProxy inDevice,
        const ThreeSampleHostOptions &options)
    {
        validateWebgpuMultipleElementsOptions(options);
        device = inDevice;
        entities.clear();
        entities.resize(40u);
        uint32_t randomState = options.randomSeed;
        for (uint32_t call = 0u; call < RandomPreamble; ++call)
            (void)nextWebgpuMultipleElementsRandom(randomState);
        ...
        for (uint32_t index = 0u; index < entities.size(); ++index)
        {
            ...
            appendWebgpuMultipleElementsCardBackground(entity);
            const uint32_t geometryIndex = static_cast<uint32_t>(nextWebgpuMultipleElementsRandom(randomState) * 4.0f);
            const float hue = nextWebgpuMultipleElementsRandom(randomState);
            if (geometryIndex == 0u) buildWebgpuMultipleElementsBox(entity);
            else if (geometryIndex == 1u) buildWebgpuMultipleElementsSphere(entity);
            else if (geometryIndex == 2u) buildWebgpuMultipleElementsDodecahedron(entity);
            else buildWebgpuMultipleElementsCylinder(entity);
            ...
            entity.materialData.baseColorAndFlags = glm::vec4(webgpuMultipleElementsHslToLinear(hue), 1.0f);
        }
        for (uint32_t index = 0u; index < entities.size(); ++index)
        {
            const auto encoder = renderer.createRenderSetCommandEncoder(SceneRenderSetHandles[index]);
            if (!encoder) throw std::runtime_error("webgpu_multiple_elements could not create its RenderSet encoder.");
            ...
            appendWebgpuMultipleElementsBuffer(allocation, WebgpuMultipleElementsSceneRenderSetComponents::vertices, "WebgpuMultipleElementsVertices", entity.vertices.data(), entity.vertices.size() * sizeof(entity.vertices[0]), 1u);
            appendWebgpuMultipleElementsBuffer(allocation, WebgpuMultipleElementsSceneRenderSetComponents::indices, "WebgpuMultipleElementsIndices", entity.indices.data(), entity.indices.size() * sizeof(entity.indices[0]), 1u);
            appendWebgpuMultipleElementsBuffer(allocation, WebgpuMultipleElementsSceneRenderSetComponents::objects, "WebgpuMultipleElementsObject", &entity.objectData, sizeof(entity.objectData), 1u);
            appendWebgpuMultipleElementsBuffer(allocation, WebgpuMultipleElementsSceneRenderSetComponents::instances, "WebgpuMultipleElementsInstance", &entity.instanceData, sizeof(entity.instanceData), 1u);
            appendWebgpuMultipleElementsBuffer(allocation, WebgpuMultipleElementsSceneRenderSetComponents::materials, "WebgpuMultipleElementsMaterial", &entity.materialData, sizeof(entity.materialData), 1u);
            encoder->allocEntity(allocation);
            renderer.executeRenderSetCommand(SceneRenderSetHandles[index], encoder);
        }
    }

    void WebgpuMultipleElementsRuntimeAdapter::beforeFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex)
    {
    ...
    void WebgpuMultipleElementsRuntimeAdapter::afterFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex,
        ...
        uint32_t height)
    {
        ...
        prepareWebgpuMultipleElementsPath(options.captureRgbaPath);
        ...
        prepareWebgpuMultipleElementsPath(options.captureMetadataPath);
        if (!options.captureMetadataPath.empty())
        {
            ...
            output << "{\n  \"schemaVersion\": 1,\n"
                   << "  \"caseId\": \"webgpu_multiple_elements\",\n"
            ...
            if (replayScenario)
            {
                output << "{\"schemaVersion\":1,\"caseId\":\"webgpu_multiple_elements\",\"scenarioId\":\""
            ...
            output << ",\n  \"sceneRenderSetCount\": 40,\n  \"renderSetType\": \"WebgpuMultipleElementsSceneRenderSet\",\n"
                   << "  \"entityCount\": 40,\n  \"instanceCounts\": [";
            ...
            for (size_t index = 0u; index < entities.size(); ++index)
            {
                ...
                output << "{\"id\":\"scene-" << std::setfill('0') << std::setw(2) << index
                       << "\",\"renderSetCount\":1,\"renderSetType\":\"WebgpuMultipleElementsSceneRenderSet\"}";
            }
            output << "],\n  \"scenePassCount\": 6,\n  \"screenPassCount\": 2,\n"
                   << "  \"drawCommandCount\": 6,\n  \"directDrawFallback\": false,\n"
                   << "  \"sampleCount\": 1,\n  \"msaaEnabled\": false\n}\n";
        }
        prepareWebgpuMultipleElementsPath(options.sceneSnapshotPath);
        if (!options.sceneSnapshotPath.empty())
        {
            ...
            for (size_t index = 0u; index < entities.size(); ++index)
            {
                ...
                passName << "WebgpuMultipleElementsScene" << std::setfill('0') << std::setw(2) << index << "Pass";
                snapshot << "{\"id\":\"scene-" << std::setfill('0') << std::setw(2) << index
                         << "\",\"renderSetCount\":1,\"renderSetId\":\"scene-" << std::setfill('0') << std::setw(2) << index
                         << "\",\"renderSetType\":\"WebgpuMultipleElementsSceneRenderSet\",\"renderableObjectCount\":1,\"entityCount\":1,"
            ...
            for (uint32_t ordinal = 0u; ordinal < 6u; ++ordinal)
            {
                ...
                snapshot << "{\"sceneRoot\":\"scene-" << std::setfill('0') << std::setw(2) << (visibleStart + ordinal)
                         << "\",\"scenePass\":\"main\",\"entityOrdinal\":" << ordinal << "}";
            }
            snapshot << "]\n}\n";
        }
        captureWritten = true;
    ...
    void WebgpuMultipleElementsRuntimeAdapter::shutdown(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options)
    {
        (void)renderer;
