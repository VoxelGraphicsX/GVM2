#include "WebgpuLightprobeCubecameraRuntimeAdapter.hpp"
...
namespace GVM::ThreeSamples
{
    namespace
    {
        ...
        constexpr const char *OrbitReplayHash =
            "81da239817cffce4b6cd259a85f64165aa3858a29851614eba7e9b2306065fa7";

        static_assert(sizeof(WebgpuLightprobeCubecameraVertex) == 32u);
        static_assert(sizeof(WebgpuLightprobeCubecameraHostObjectData) == 192u);
        static_assert(sizeof(WebgpuLightprobeCubecameraHostShData) == 16u);

        /** Reads one pinned cube face as exact bytes for identity validation. */
        eastl::vector<uint8_t> readWebgpuLightprobeCubecameraBytes(
        ...
        eastl::string calculateWebgpuLightprobeCubecameraSha256(
        ...
        double webgpuLightprobeCubecameraSrgbToLinear(uint8_t value)
        {
        ...
        void buildWebgpuLightprobeCubecameraSphere(
            eastl::vector<WebgpuLightprobeCubecameraVertex> &vertices,
        ...
        eastl::array<double, 9u> evaluateWebgpuLightprobeCubecameraBasis(
        ...
        glm::dvec3 makeWebgpuLightprobeCubecameraDirection(
            uint32_t face,
            double column,
            double row)
        {
        ...
        eastl::array<WebgpuLightprobeCubecameraHostShData, 9u>
        buildWebgpuLightprobeCubecameraSh(
            const eastl::array<RgbaImageData, 6u> &images)
        {
                ...
                for (uint32_t rowIndex = 0u; rowIndex < 256u; ++rowIndex)
                {
                    ...
                    for (uint32_t columnIndex = 0u; columnIndex < 256u; ++columnIndex)
                    {
                        ...
                        const glm::dvec3 cubeCoordinate =
                            makeWebgpuLightprobeCubecameraDirection(
                                face, column, row);
                        ...
                        const auto basis =
                            evaluateWebgpuLightprobeCubecameraBasis(direction);
                        const size_t offset =
                            (static_cast<size_t>(rowIndex) * 256u + columnIndex) * 4u;
                        const glm::dvec3 color(
                            webgpuLightprobeCubecameraSrgbToLinear(
                                image.pixels[offset]),
                            webgpuLightprobeCubecameraSrgbToLinear(
            ...
            eastl::array<WebgpuLightprobeCubecameraHostShData, 9u> result{};
            for (uint32_t coefficient = 0u; coefficient < 9u; ++coefficient)
        ...
        glm::mat4 makeWebgpuLightprobeCubecameraProjection()
        {
        ...
        void appendWebgpuLightprobeCubecameraPayload(
        ...
        void prepareWebgpuLightprobeCubecameraOutput(
            const std::filesystem::path &path)
        {
        ...
        void writeWebgpuLightprobeCubecameraBinary(
            const eastl::string &path,
            const eastl::vector<uint8_t> &bytes)
        {
            if (path.empty()) return;
            const std::filesystem::path outputPath(path.c_str());
            prepareWebgpuLightprobeCubecameraOutput(outputPath);
        ...
        void writeWebgpuLightprobeCubecameraText(
            const eastl::string &path,
    ...
    void WebgpuLightprobeCubecameraRuntimeAdapter::initializeResources(
        ...
        const ThreeSampleHostOptions &options)
    {
        ...
        if (options.caseId != "webgpu_lightprobe_cubecamera" ||
            (!initial && !orbit) ||
            ...
            throw std::invalid_argument(
                "WebGPU LightProbe cube-camera requires the locked r185 scenarios.");
        if (orbit && calculateWebgpuLightprobeCubecameraSha256(
                readWebgpuLightprobeCubecameraBytes(
                    std::filesystem::path(options.inputReplayPath.c_str()))) !=
                OrbitReplayHash)
            throw std::invalid_argument(
                "WebGPU LightProbe cube-camera replay differs from its pinned identity.");
        device = inDevice;
        buildWebgpuLightprobeCubecameraSphere(vertices, indices);
        ...
        for (uint32_t face = 0u; face < 6u; ++face)
        {
            ...
            const eastl::vector<uint8_t> encoded =
                readWebgpuLightprobeCubecameraBytes(facePath);
            if (calculateWebgpuLightprobeCubecameraSha256(encoded) != CubeHashes[face])
        ...
        shData = buildWebgpuLightprobeCubecameraSh(baseImages);
        ...
        objectData.projection = makeWebgpuLightprobeCubecameraProjection();
        ...
        appendWebgpuLightprobeCubecameraPayload(
            allocation,
            WebgpuLightprobeCubecameraSceneRenderSetComponents::vertices,
            "WebgpuLightprobeCubecameraVertices",
            vertices.data(), vertices.size() * sizeof(vertices[0]), 1u);
        appendWebgpuLightprobeCubecameraPayload(
            allocation,
            WebgpuLightprobeCubecameraSceneRenderSetComponents::indices,
            "WebgpuLightprobeCubecameraIndices",
            indices.data(), indices.size() * sizeof(indices[0]), 1u);
        appendWebgpuLightprobeCubecameraPayload(
            allocation,
            WebgpuLightprobeCubecameraSceneRenderSetComponents::objects,
            "WebgpuLightprobeCubecameraObject",
            &objectData, sizeof(objectData), 1u);
        appendWebgpuLightprobeCubecameraPayload(
            allocation,
            WebgpuLightprobeCubecameraSceneRenderSetComponents::instances,
            "WebgpuLightprobeCubecameraInstance",
            &instanceData, sizeof(instanceData), 1u);
        appendWebgpuLightprobeCubecameraPayload(
            allocation,
            WebgpuLightprobeCubecameraSceneRenderSetComponents::materials,
            "WebgpuLightprobeCubecameraMaterial",
            &materialData, sizeof(materialData), 1u);
        appendWebgpuLightprobeCubecameraPayload(
            allocation,
            WebgpuLightprobeCubecameraSceneRenderSetComponents::sphericalHarmonics,
            "WebgpuLightprobeCubecameraSphericalHarmonics",
            shData.data(), sizeof(shData), 9u);
        encoder->allocEntity(allocation);
        renderer.executeRenderSetCommand(SceneRenderSetHandle, encoder);
    }

    void WebgpuLightprobeCubecameraRuntimeAdapter::beforeFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex)
    {
    ...
    void WebgpuLightprobeCubecameraRuntimeAdapter::afterFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex,
        ...
        uint32_t height)
    {
        ...
        device->graphicsQueue(0)
            ...
            ->submit();
        writeWebgpuLightprobeCubecameraBinary(options.captureRgbaPath, rgba);
        std::ostringstream metadata;
        metadata
            << "{\"schemaVersion\":1,\"source\":\"gvm-three-r185\","
            << "\"caseId\":\"webgpu_lightprobe_cubecamera\",\"scenarioId\":\""
        ...
        if (options.scenarioId == "orbit-after-probe")
            metadata
                ...
                << "\"target\":\"body > canvas\"}";
        metadata << "}\n";
        writeWebgpuLightprobeCubecameraText(
            options.captureMetadataPath, metadata.str());
        std::ostringstream snapshot;
        snapshot
            << "{\"schemaVersion\":1,\"caseId\":\"webgpu_lightprobe_cubecamera\","
            ...
            << "\"WebgpuLightprobeCubecameraSceneRenderSet\",\"sceneRoots\":[{"
            << "\"id\":\"scene\",\"renderSetCount\":1,"
            << "\"renderSetId\":\"scene-set\",\"renderSetType\":"
            << "\"WebgpuLightprobeCubecameraSceneRenderSet\","
        ...
        writeWebgpuLightprobeCubecameraText(options.sceneSnapshotPath, snapshot.str());
        writeWebgpuLightprobeCubecameraText(options.semanticSnapshotPath, snapshot.str());
        captureWritten = true;
    }

    void WebgpuLightprobeCubecameraRuntimeAdapter::shutdown(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options)
    {
        (void)renderer;
