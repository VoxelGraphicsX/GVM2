...
const caseId = 'webgl_postprocessing_afterimage';
const shardName = 'Phase1WebglPostprocessingAfterimageRenderSet';
const replayDefinitions = Object.freeze([
  {
    relativePath: path.join(
      'inputs', 'webgl_postprocessing_afterimage_damp.json'),
    sha256: '1a6b37c553e4b3225e1f3306972a40cdade7810382cf5e72ee9e04a796c2bb4b'
  },
  {
    relativePath: path.join(
      'inputs', 'webgl_postprocessing_afterimage_disabled.json'),
    sha256: '3ce251b81f5312bfe6b5b1e89ddaf15ccd19b7b62c3f973e453bc9e121e6ed74'
  }
]);
const generatedScreenPasses = Object.freeze([
  'WebglPostprocessingAfterimageHistoryClearPass',
  'WebglPostprocessingAfterimageComposePass',
  'WebglPostprocessingAfterimageCopyPass',
...
/** Loads the dedicated Afterimage example from the frozen Manifest. */
async function loadSelectedExample(manifestPath) {
  ...
  if (!Array.isArray(manifest.examples) || manifest.examples.length !== 588) {
    throw new Error(
      'Afterimage fixture requires the frozen 588-item Three r185 Manifest.');
  }
  ...
  if (example?.status !== 'phase1_required'
      ...
      || example.renderSetType
        !== 'WebglPostprocessingAfterimageSceneRenderSet'
      || example.scenarios?.length !== 4) {
    throw new Error(
...
/** Copies and verifies deterministic Afterimage replays into the asset root. */
async function stageInputs(outputDirectory) {
  ...
  for (const definition of replayDefinitions) {
    ...
    if (sha256 !== definition.sha256) {
      throw new Error(
        `${sourcePath}: replay identity differs from the locked contract.`);
    }
...
function createGeneratedArtifactReport(generatedRoot, manifestPath) {
  const result = lintGeneratedArtifacts({
    ...
    manifestPath,
    caseIds: [caseId],
    fixtureSelectors: generatedScreenPasses.map((name) => `${name}:screen`),
    instancingPasses: []
  });
  return {
...
/** Runs every Afterimage scenario through the full strict matrix. */
...
/** Executes the strict dedicated Afterimage fixture and writes summaries. */
async function main() {
