#include "WebglDepthTextureRuntimeAdapter.hpp"
...
namespace GVM::ThreeSamples
{
    namespace
    {
        constexpr uint32_t DepthInstanceCount = 50u;
        ...
        // The locked r185 module graph consumes 160 UUID draws before the
        // InstancedMesh transform loop. The loop itself consumes five draws per instance.
        constexpr uint32_t PreSceneRandomCallCount = 160u;
        constexpr uint32_t InitialSceneRandomState = 4279293364u;
        ...
        constexpr float OrbitDampedDragFactor = 0.0975f;
        ...
        static_assert(sizeof(WebglDepthTextureHostVertex) == 16u);
        static_assert(sizeof(WebglDepthTextureHostObjectData) == 64u);
        static_assert(sizeof(WebglDepthTextureHostInstanceData) == 64u);
        static_assert(sizeof(WebglDepthTextureHostMaterialData) == 16u);
            ...
            eastl::vector<uint32_t> &indices)
        {
            ...
            if (vertices.size() != TorusKnotVertexCount || indices.size() != TorusKnotIndexCount)
                throw std::runtime_error("webgl_depth_texture TorusKnot counts diverged from r185.");
        }

        /** Builds the zero-to-one, Y-inverted projection matching the WebGL camera. */
        glm::mat4 makeDepthPerspective(uint32_t width, uint32_t height)
        {
            constexpr double nearDistance = 0.01;
            constexpr double farDistance = 50.0;
        ...
        /** Returns one seeded InstancedMesh model transform. */
        glm::mat4 makeDepthInstanceTransform(ThreeCompat::DeterministicRandom &random)
        {
        ...
        void validateOptions(const ThreeSampleHostOptions &options)
        {
            ...
            if (options.caseId != "webgl_depth_texture" || (!initial && !orbit) ||
                options.width != 800u || options.height != 500u ||
                options.randomSeed != DefaultThreeRandomSeed)
                throw std::invalid_argument("webgl_depth_texture scenario does not match the locked r185 contract.");
        }
    } // namespace

    void WebglDepthTextureRuntimeAdapter::initializeResources(
        ...
        const ThreeSampleHostOptions &options)
    {
        ...
        eastl::vector<WebglDepthTextureHostVertex> canonicalVertices;
        ...
        entity.instanceData.clear();
        entity.instanceData.reserve(DepthInstanceCount);
        entity.materialData.baseColor = glm::vec4(0.0f, 0.0f, 1.0f, 1.0f);
        ThreeCompat::DeterministicRandom random(DefaultThreeRandomSeed);
        advanceToDepthSceneRandomStream(random);
        for (uint32_t index = 0u; index < DepthInstanceCount; ++index)
            entity.instanceData.push_back({makeDepthInstanceTransform(random)});
        if (random.getState() != InitialSceneRandomState)
            throw std::runtime_error("webgl_depth_texture random stream diverged from the locked r185 instance stream.");
        const auto encoder = renderer.createRenderSetCommandEncoder(SceneRenderSetHandle);
        if (!encoder) throw std::runtime_error("webgl_depth_texture could not create its Scene Set encoder.");
        ...
        allocation.instanceCount = DepthInstanceCount;
        const eastl::string prefix = "DepthInstancedMesh";
        appendBuffer(allocation, WebglDepthTextureSceneRenderSetComponents::vertices,
                     prefix + "-vertices", entity.vertices.data(),
                     entity.vertices.size() * sizeof(WebglDepthTextureHostVertex), 1u);
        appendBuffer(allocation, WebglDepthTextureSceneRenderSetComponents::indices,
                     prefix + "-indices", entity.indices.data(),
                     entity.indices.size() * sizeof(uint32_t), 1u);
        appendBuffer(allocation, WebglDepthTextureSceneRenderSetComponents::objects,
                     prefix + "-object", &entity.objectData, sizeof(entity.objectData), 1u);
        appendBuffer(allocation, WebglDepthTextureSceneRenderSetComponents::instances,
                     prefix + "-instances", entity.instanceData.data(),
                     entity.instanceData.size() * sizeof(WebglDepthTextureHostInstanceData),
                     DepthInstanceCount);
        appendBuffer(allocation, WebglDepthTextureSceneRenderSetComponents::materials,
    ...
    void WebglDepthTextureRuntimeAdapter::updateObjectData(uint32_t frameIndex)
    {
        viewProjection = makeDepthViewProjection(800u, 500u, frameIndex);
        entity.objectData.viewProjection = viewProjection;
    }

    void WebglDepthTextureRuntimeAdapter::beforeFrame(GVM::Core::AbstractRendererImpl &renderer,
                                                       const ThreeSampleHostOptions &options,
                                                       uint32_t frameIndex)
    {
        (void)options;
        ...
        const auto encoder = renderer.createRenderSetCommandEncoder(SceneRenderSetHandle);
        if (!encoder) throw std::runtime_error("webgl_depth_texture could not create its update encoder.");
        encoder->setBufferComponentData(entity.entityIndex,
            WebglDepthTextureSceneRenderSetComponents::objects,
            &entity.objectData, sizeof(entity.objectData), 0u, 1u);
        renderer.executeRenderSetCommand(SceneRenderSetHandle, encoder);
    }

    void WebglDepthTextureRuntimeAdapter::writeRgbaCapture(const ThreeSampleHostOptions &options,
    ...
    void WebglDepthTextureRuntimeAdapter::writeCaptureMetadata(const ThreeSampleHostOptions &options,
                                                                ...
                                                                uint32_t height, uint64_t byteCount) const
    {
        ...
        output << "{\"schemaVersion\":1,\"source\":\"gvm-three-r185\",\"caseId\":\"webgl_depth_texture\",\"scenarioId\":\""
        ...
        if (!options.inputReplayPath.empty())
        {
            output << ",\"inputReplay\":{\"sha256\":\""
                   ...
                   << "\",\"caseId\":\"webgl_depth_texture\",\"scenarioId\":\""
                   << options.scenarioId.c_str() << "\",\"captureFrame\":1,\"eventCount\":3,\"target\":\"body > canvas\"}";
        }
        output << "}\n";
    }

    void WebglDepthTextureRuntimeAdapter::writeStructuralSnapshot(const ThreeSampleHostOptions &options,
                                                                   uint32_t frameIndex) const
    {
        ...
        preparePath(path);
        std::ofstream output(path, std::ios::trunc);
        output << "{\n  \"schemaVersion\":1,\n  \"caseId\":\"webgl_depth_texture\",\n  \"scenarioId\":\""
               << options.scenarioId.c_str() << "\",\n  \"frame\":" << frameIndex
               << ",\n  \"implementationLevel\":\"semantic-complete\",\n  \"gpuWorkDslOnly\":true,\n"
               << "  \"assetBacked\":true,\n  \"renderSetPolicy\":\"required\",\n  \"sceneRenderSetCount\":1,\n"
               << "  \"renderSetType\":\"WebglDepthTextureSceneRenderSet\",\n  \"renderableObjectCount\":1,\n"
               << "  \"entityCount\":1,\n  \"instanceCount\":50,\n  \"instanceCounts\":[50],\n"
               << "  \"vertexCount\":8385,\n  \"indexCount\":49152,\n"
               << "  \"scenePassCount\":1,\n  \"screenPassCount\":1,\n  \"drawCommandCount\":1,\n"
               ...
               << "  \"assetAndAlgorithmState\":\"r185-instanced-torus-knot-depth32float-linearized-single-sample\",\n"
               << "  \"scenePasses\":[{\"name\":\"depth-color\",\"renderClass\":\"WebglDepthTextureScenePass\",\"renderSetId\":\"scene-set\",\"renderSetBindingCount\":1,\"drawMode\":\"render-set-indexed-indirect\",\"invocationCount\":1,\"drawCommandCount\":1,\"usesStandaloneGeometry\":false,\"usesExplicitDrawCount\":false}],\n"
               << "  \"screenPasses\":[{\"name\":\"depth-linearization-and-visualization\",\"renderClass\":\"WebglDepthTextureScreenPass\",\"samplesDepthTexture\":true,\"redrawsSceneGeometry\":false}],\n"
               << "  \"sceneRoots\":[{\"id\":\"scene\",\"renderSetCount\":1,\"renderSetId\":\"scene-set\",\"renderSetType\":\"WebglDepthTextureSceneRenderSet\",\"renderableObjectCount\":1,\"entityCount\":1,\"entities\":[";
        output << "{\"entityId\":0,\"logicalRenderableId\":\"depth-instanced-mesh\",\"instanceCount\":50}";
        output << "],\"componentSchema\":[{\"name\":\"vertices\",\"kind\":\"buffer\",\"role\":\"vertex\"},{\"name\":\"indices\",\"kind\":\"buffer\",\"role\":\"index\"},{\"name\":\"objects\",\"kind\":\"buffer\",\"role\":\"object\"},{\"name\":\"instances\",\"kind\":\"buffer\",\"role\":\"instance\"},{\"name\":\"materials\",\"kind\":\"buffer\",\"role\":\"material\"}],\"drawCommandCount\":1,\"directDrawFallback\":false,\"scenePasses\":[{\"name\":\"depth-color\",\"renderClass\":\"WebglDepthTextureScenePass\",\"renderSetId\":\"scene-set\",\"renderSetBindingCount\":1,\"drawMode\":\"render-set-indexed-indirect\",\"invocationCount\":1,\"drawCommandCount\":1,\"usesStandaloneGeometry\":false,\"usesExplicitDrawCount\":false}]},{\"id\":\"postScene\",\"renderSetCount\":0,\"renderSetId\":null,\"renderSetType\":null,\"renderableObjectCount\":0,\"entityCount\":0,\"entities\":[],\"componentSchema\":[],\"drawCommandCount\":0,\"directDrawFallback\":false,\"scenePasses\":[]}],\"scenePassSequence\":[{\"sceneRoot\":\"scene\",\"scenePass\":\"depth-color\",\"entityOrdinal\":0}]}\n";
    }

    void WebglDepthTextureRuntimeAdapter::afterFrame(GVM::Core::AbstractRendererImpl &renderer,
                                                      const ThreeSampleHostOptions &options,
                                                      ...
                                                      uint32_t width, uint32_t height)
    {
        ...
        if (byteCount > std::numeric_limits<size_t>::max()) throw std::overflow_error("webgl_depth_texture capture is too large.");
        eastl::vector<uint8_t> rgba(static_cast<size_t>(byteCount));
        const auto queue = device->graphicsQueue(0);
        if (!queue) throw std::runtime_error("webgl_depth_texture has no graphics queue.");
    ...
    void WebglDepthTextureRuntimeAdapter::shutdown(GVM::Core::AbstractRendererImpl &renderer,
                                                    const ThreeSampleHostOptions &options)
    {
        (void)renderer;
        (void)options;
