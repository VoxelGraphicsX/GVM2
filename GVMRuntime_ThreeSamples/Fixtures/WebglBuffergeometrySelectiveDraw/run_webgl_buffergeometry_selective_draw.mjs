...
const shardName = 'WebglBuffergeometrySelectiveDraw';
const caseId = 'webgl_buffergeometry_selective_draw';
const randomSeed = 0x1850000e;
const repeatCount = 3;
const upstreamCommit = '2431a09f46f34c560bc8e44b33be0e567723d5b9';
const upstreamSourceSha256 = 'e12561e5f269a6e9ef4659d914eddf1bfcd54d01f2e60cfcc89512e2bedcf927';
const replaySha256 = '6d38e4787716906c619f68ccfc21b24880ca38d86229e7fc421d656e0792d7b2';
const stateScriptSha256 = 'f3be5c6fcbf5bf21fb69248f5369f7bfe76492692ab888f5a61e6968404edf53';

export const selectiveDrawScenarios = Object.freeze([
  Object.freeze({
    id: 'initial-all-visible',
    frame: 0,
    canonicalState: 'seed-0x1850000e-all-20000-lines-visible',
    scenePass: 'main-selective-lines',
    renderClass: 'WebglBuffergeometrySelectiveDrawMainPass',
    culled: false,
  ...
  Object.freeze({
    id: 'animated',
    frame: 60,
    canonicalState: 'seed-0x1850000e-fixed-step-60hz-all-visible',
    scenePass: 'main-selective-lines',
    renderClass: 'WebglBuffergeometrySelectiveDrawMainPass',
    culled: false,
    visibleLineCount: 20000,
  ...
  Object.freeze({
    id: 'culled',
    frame: 61,
    canonicalState: 'canonical-hide-lines-click-4986-culled-15014-visible',
    scenePass: 'main-selective-lines',
    renderClass: 'WebglBuffergeometrySelectiveDrawMainPass',
    culled: true,
...
export const selectiveDrawParityDefinitions = Object.freeze([
  Object.freeze({
...
const lockedOracleSha256 = Object.freeze({
  'initial-all-visible': Object.freeze({
    rgba: 'd95ba8e02d159d14ed7de83b8bcaa0e827f1e9e65590a31f95ffbe5279861404',
    json: '98a409c147fe2fed600b4e5ec1b18905f5e10299922624730270c642dfaec27a'
  }),
  animated: Object.freeze({
    rgba: 'bf4fe07bcc5ea4d4a6b497aa47fbad32a093839abf32f644fa2ab4a5aaba5d09',
    json: 'b54136ac2cfdbd20aa07b7b0d925cc5f45a844ba303c2483df05ff0917db5892'
  }),
  culled: Object.freeze({
    rgba: 'ae3c5565b0792f19bdf92c6b890d5b824ecdc0238c8846c531d632d2d1c14496',
    json: '4c9d1fbcf8a60eff42a0543c967cb9a4fb81a520192d9b37ab96b4276a6a2f4d'
  })
});
...
export function validateManifestContract(example) {
  ...
  validateExpectedFields(example, {
    id: caseId,
    upstreamPath: 'examples/webgl_buffergeometry_selective_draw.html',
    ...
    componentSchema: [],
    dslShard: shardName,
    scenarios: selectiveDrawScenarios.map((scenario) => ({
      id: scenario.id,
      frame: scenario.frame,
      inputReplay: scenario.replay
        ? 'inputs/webgl_buffergeometry_selective_draw_hide.json'
        : null,
  ...
  for (const capability of [
    'triangle_list_screen_space_line_expansion',
    'custom_visibility_vertex_attribute',
...
export function buildExpectedSnapshot(scenario) {
  return {
    ...
    sourceLineCount: 20000,
    sourceVertexCount: 40000,
    expandedVertexCount: 120000,
    visibleLineCount: scenario.visibleLineCount,
    culledLineCount: scenario.culledLineCount,
    preGeometryRandomCallCount: 88,
    canonicalHidePreVisibilityRandomCallCount: 44,
    canonicalHidePreVisibilityRandomState: 2347915795,
    canonicalHidePostVisibilityRandomState: 248151130,
    ...
    vertexBufferUploadCount: scenario.culled ? 2 : 1,
    initialVisibilityUploadAllVisible: true,
    replayVisibilityUpdateApplied: scenario.culled,
    visibilityReplayApplied: scenario.culled,
...
/** Validates immutable Oracle identity, timing, and hide-replay provenance. */
...
function extractGeneratedAbiSignature(generatedSource) {
  const strideMatches = [...generatedSource.matchAll(/arrayStride\s*=\s*60;/gu)]
    .map((match) => Number(match[1]));
  return {
    vertexStrideCount: strideMatches.length,
    vertexAttributeOffsets: [
      'startPosition', 'endPosition', 'startColor', 'endColor', 'lineCoordinate', 'visible'
    ].map((name) => generatedSource.includes(
      `offsetof(WebglBuffergeometrySelectiveDrawVertex, ${name})`))
  };
}

/** Lints one generated pipeline for the frozen expanded-line ABI and DSL boundary. */
export function validateGeneratedSourceContract(pipeline, generatedSource, singleHeader) {
  ...
  for (const forbidden of [
    'computePass("WebglBuffergeometrySelectiveDrawResolve"',
    'renderPass("WebglBuffergeometrySelectiveDrawSample1"',
    'renderPass("WebglBuffergeometrySelectiveDrawSample2"',
    'renderPass("WebglBuffergeometrySelectiveDrawSample3"'
  ]) {
  ...
  if (abiSignature.vertexStrideCount !== 1 ||
      abiSignature.vertexAttributeOffsets.includes(false)) {
...
export function validateGeneratedAbiParity(entries) {
  ...
  if (entries.length !== 2 || entries.some((entry) => entry.status !== 'pass')) {
    ...
    failures.push('Legacy and Experimental selective line vertex-layout ABI signatures differ.');
  }
...
/** Loads both generated families and all three Experimental shader products. */
  ...
  for (const pipeline of pipelines) {
    ...
    if (pipeline === 'experimental') {
      const stageDefinitions = [
        ['WebglBuffergeometrySelectiveDrawMainPass__vertex', 'vertex'],
        ['WebglBuffergeometrySelectiveDrawMainPass__fragment', 'fragment']
      ];
      const products = [];
...
async function compareScenarioParity(scenario, canonicalRuns) {
  const comparisons = [];
  for (const definition of selectiveDrawParityDefinitions) {
...
async function validateInputs(context, manifest) {
  ...
  const upstreamSourcePath = path.join(
    context.upstreamRoot, 'examples', 'webgl_buffergeometry_selective_draw.html');
  ...
  const oracles = [];
  for (const scenario of selectiveDrawScenarios) {
...
/** Executes the complete 36-run selective-line gate and writes one summary report. */
async function main() {
  ...
  const expectedRunCount =
    selectiveDrawScenarios.length * pipelines.length * backends.length * repeatCount;
  const expectedStabilityCount =
    selectiveDrawScenarios.length * pipelines.length * backends.length;
  const expectedCrossComparisonCount =
    selectiveDrawScenarios.length * selectiveDrawParityDefinitions.length;
  ...
  const report = {
    schemaVersion: 1,
    gate: 'three-r185-webgl-buffergeometry-selective-draw',
    caseId,
    status,
    comparisonThresholds,
    matrix: {
      pipelines,
      backends,
      scenarios: selectiveDrawScenarios.map((scenario) => scenario.id),
      repeatCount,
      expectedRunCount,
  ...
  console.log(`webgl_buffergeometry_selective_draw report: ${reportPath}`);
