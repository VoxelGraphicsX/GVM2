...
namespace GVM::ThreeSamples
{
    ...
    struct alignas(16) WebgpuPostprocessingMaskingHostObjectData
    {
        glm::mat4 modelViewProjection;
    };

    /** Mirrors the mandatory identity instance component. */
    struct alignas(16) WebgpuPostprocessingMaskingHostInstanceData
    {
        glm::vec4 reserved;
    };

    /** Mirrors the opaque mask material phase. */
    struct alignas(16) WebgpuPostprocessingMaskingHostMaterialData
    {
        glm::vec4 maskAndPhase;
    };

    /** Connects both masking Scenes and pinned photographs to the dedicated Renderer. */
    class WebgpuPostprocessingMaskingRuntimeAdapter final
    {
    public:
        /** Builds both target-frame meshes, allocates maskScene1, and uploads photos. */
    ...
    private:
        ...
        eastl::vector<WebgpuPostprocessingMaskingVertex> boxVertices;
        eastl::vector<uint32_t> boxIndices;
        eastl::vector<WebgpuPostprocessingMaskingVertex> torusVertices;
        eastl::vector<uint32_t> torusIndices;
        WebgpuPostprocessingMaskingTorusUniforms torusUniforms{};
        eastl::vector<eastl::vector<uint8_t>> photograph1Mips;
        eastl::vector<eastl::vector<uint8_t>> photograph2Mips;
        WebgpuPostprocessingMaskingHostObjectData boxObjectData{};
        WebgpuPostprocessingMaskingHostInstanceData boxInstanceData{};
        WebgpuPostprocessingMaskingHostMaterialData boxMaterialData{};
