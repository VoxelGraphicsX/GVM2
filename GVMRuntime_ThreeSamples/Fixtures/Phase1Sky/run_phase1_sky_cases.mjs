...
const caseConfigurations = Object.freeze([
  Object.freeze({
    caseId: 'webgl_shaders_sky',
    shardName: 'Phase1WebglShadersSkyRenderSet',
    scenePass: 'WebglShadersSkyScenePass',
    renderSetType: 'WebglShadersSkySceneRenderSet',
    entityCount: 2
  }),
  Object.freeze({
    caseId: 'webgpu_sky',
    shardName: 'Phase1WebgpuSkyRenderSet',
    scenePass: 'WebgpuSkyScenePass',
    renderSetType: 'WebgpuSkySceneRenderSet',
    entityCount: 1
  })
]);
...
/** Loads and validates both dedicated sky examples from the frozen Manifest. */
async function loadSelectedExamples(manifestPath) {
  ...
  if (!Array.isArray(manifest.examples) || manifest.examples.length !== 588) {
    throw new Error(
      'Sky fixture requires the frozen 588-item Three r185 Manifest.'
    );
  }
  return caseConfigurations.map((configuration) => {
    ...
    if (example?.status !== 'phase1_required'
        ...
        || example.renderableObjectCount !== configuration.entityCount
        || example.scenePasses?.length !== 1
        || example.scenePasses[0]?.renderClass
          !== configuration.scenePass
        || example.scenarios?.length !== 3) {
      throw new Error(
        `${configuration.caseId}: dedicated sky Manifest contract changed.`
      );
    }
    return example;
  });
...
/** Stages the immutable sky input replays below the explicit asset root. */
async function stageFixtureAssets(outputDirectory) {
  ...
  for (const caseId of caseIds) {
    const fileName = `${caseId}_controls.json`;
    await fs.copyFile(
      path.join(
        repositoryRoot,
...
/** Creates the independent generated-host selection for one sky example. */
...
/** Runs all sky scenarios through every pipeline, backend, and repetition. */
...
/** Builds one independently gateable strict summary for a sky example. */
...
/** Executes the strict dedicated sky fixture and writes aggregate summaries. */
async function main() {
  ...
  const perCaseExpectedRunCount =
    3 * requiredPipelines.length * requiredBackends.length * repetitions;
  ...
  console.log(
    `Sky fixture ${status}: ${path.join(outputDirectory, 'summary.json')}`
  );
  if (status !== 'pass') {
    process.exitCode = 1;
  }
