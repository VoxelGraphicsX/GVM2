#include "WebgpuMultipleRendertargetsReadbackRuntimeAdapter.hpp"
...
namespace GVM::ThreeSamples
{
    namespace
    {
        ...
        /** Validates the four deterministic MRT readback scenarios. */
        void validateMultipleRendertargetsScenario(
            const ThreeSampleHostOptions &options)
        {
            const bool initial =
                options.scenarioId == "initial-mrt" &&
                options.targetFrame == 0u;
            const bool animated =
                options.scenarioId == "animated-mrt" &&
                options.targetFrame == 60u;
            const bool diffuseReadback =
                options.scenarioId == "diffuse-readback" &&
                options.targetFrame == 61u;
            const bool normalReadback =
                options.scenarioId == "normal-readback" &&
                options.targetFrame == 62u;
            if (options.caseId != "webgpu_multiple_rendertargets_readback" ||
                (!initial && !animated && !diffuseReadback && !normalReadback) ||
                options.width != 800u ||
                options.height != 500u ||
                options.randomSeed != DefaultThreeRandomSeed ||
                ((diffuseReadback || normalReadback) !=
                 !options.inputReplayPath.empty()))
            {
                throw std::invalid_argument(
                    "WebGPU MRT readback requires one locked Manifest scenario.");
            }
        }
    ...
    void WebgpuMultipleRendertargetsReadbackRuntimeAdapter::initializeResources(
        ...
        const ThreeSampleHostOptions &options)
    {
        ...
        outputSelection = options.scenarioId == "diffuse-readback"
            ? 1.0f
            : options.scenarioId == "normal-readback" ? 2.0f : 0.0f;
    }

    void WebgpuMultipleRendertargetsReadbackRuntimeAdapter::beforeFrame(
    ...
    void WebgpuMultipleRendertargetsReadbackRuntimeAdapter::captureTargetFrame(
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex,
        ...
        uint32_t height)
    {
        ...
        metadata
            ...
            << ",\n  \"byteCount\": " << byteCount
            << ",\n  \"format\": \"rgba8unorm\",\n  \"inputReplay\": ";
        if (options.scenarioId == "diffuse-readback")
        {
            metadata
                << "{\"sha256\":\"b23b2a34a1399bec4679916683168f23543922ad1d4539d7e48f0380bd2882b3\","
                << "\"caseId\":\"webgpu_multiple_rendertargets_readback\","
                << "\"scenarioId\":\"diffuse-readback\",\"captureFrame\":61,"
                << "\"eventCount\":1,\"target\":\"canvas:not([class])\"}";
        }
        else if (options.scenarioId == "normal-readback")
        {
            metadata
                << "{\"sha256\":\"925455733c688943742e5f0e1c9ef6e314a95f2773f945f6ea6d16b00c0a5e98\","
                << "\"caseId\":\"webgpu_multiple_rendertargets_readback\","
                << "\"scenarioId\":\"normal-readback\",\"captureFrame\":62,"
                << "\"eventCount\":1,\"target\":\"canvas:not([class])\"}";
        }
        else
        {
        ...
        writeMultipleRendertargetsText(
            ...
            "\"scenePass\":\"output-normal-readback-mrt\",\"entityOrdinal\":0}]\n}\n");
        writeMultipleRendertargetsText(
            ...
            "  \"readbackExtent\": [512, 512],\n"
            "  \"screenPasses\": [\"mrt-direct-composite\","
            "\"attachment-readback-upload\",\"readback-fullscreen-output\"]\n}\n");
        captureWritten = true;
    }

    void WebgpuMultipleRendertargetsReadbackRuntimeAdapter::shutdown(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options)
    {
        (void)renderer;
