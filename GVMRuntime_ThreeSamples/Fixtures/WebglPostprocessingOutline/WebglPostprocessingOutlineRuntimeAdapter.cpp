#include "WebglPostprocessingOutlineRuntimeAdapter.hpp"
...
namespace GVM::ThreeSamples
{
    namespace
    {
        ...
        constexpr uint32_t RenderableCount = 23u;

        static_assert(sizeof(WebglPostprocessingOutlineVertex) == 32u);
        static_assert(sizeof(WebglPostprocessingOutlineHostObjectData) == 192u);
        static_assert(sizeof(WebglPostprocessingOutlineHostInstanceData) == 16u);
        static_assert(sizeof(WebglPostprocessingOutlineHostMaterialData) == 32u);

        /** Creates parent directories for one requested evidence path. */
        void prepareWebglPostprocessingOutlineOutput(
        ...
        void writeWebglPostprocessingOutlineText(
            const eastl::string &path,
            const std::string &text)
        {
            if (path.empty()) return;
            const std::filesystem::path outputPath(path.c_str());
            prepareWebglPostprocessingOutlineOutput(outputPath);
        ...
        double nextWebglPostprocessingOutlineRandom(uint32_t &state)
        {
            state ^= state << 13u;
            state ^= state >> 17u;
            state ^= state << 5u;
        ...
        glm::dmat4 makeWebglPostprocessingOutlineRotation(
            double rotationX,
            double rotationY,
            double rotationZ)
        {
        ...
        glm::dmat4 makeWebglPostprocessingOutlineProjection()
        {
        ...
        void appendWebglPostprocessingOutlineTriangle(
            const glm::dvec3 &a,
            const glm::dvec3 &b,
            const glm::dvec3 &c,
            eastl::vector<WebglPostprocessingOutlineVertex> &vertices,
        ...
        void buildWebglPostprocessingOutlineSphere(
            eastl::vector<WebglPostprocessingOutlineVertex> &vertices,
            eastl::vector<uint32_t> &indices)
        {
            ...
            for (uint32_t y = 0u; y < 4u; ++y)
            {
                for (uint32_t x = 0u; x < 4u; ++x)
                {
                    ...
                    if (y != 0u)
                        appendWebglPostprocessingOutlineTriangle(
        ...
        void appendWebglPostprocessingOutlinePayload(
    ...
    void WebglPostprocessingOutlineRuntimeAdapter::initializeResources(
        ...
        const ThreeSampleHostOptions &options)
    {
        ...
        const bool scenario2 = options.scenarioId == "selected-pattern" && options.targetFrame == 1u;
        const bool scenario3 = options.scenarioId == "rotated-pulse" && options.targetFrame == 120u;
        if (options.caseId != "webgl_postprocessing_outline" ||
            !(scenario0 || scenario1 || scenario2 || scenario3) ||
            options.width != 800u || options.height != 500u ||
            options.randomSeed != DefaultThreeRandomSeed ||
            !options.inputReplayPath.empty())
        {
            throw std::invalid_argument("webgl_postprocessing_outline requires its locked r185 scenario matrix.");
        }
        device = inDevice;
        buildWebglPostprocessingOutlineSphere(vertices, indices);
        ...
        for (uint32_t draw = 0u; draw < 140u; ++draw)
            (void)nextWebglPostprocessingOutlineRandom(randomState);
        // The upstream example uses performance.now()*0.0001 for the group
        // Y rotation; the deterministic clock advances at 60 Hz per frame.
        const double parentRotationX = 0.0;
        const double parentRotationY = double(options.targetFrame) / 60.0 * 0.1;
        const glm::dmat4 parentRotation = makeWebglPostprocessingOutlineRotation(
            parentRotationX, parentRotationY, 0.0);
        glm::dmat4 view(1.0);
        view[3u][2u] = -400.0;
        const glm::dmat4 projection = makeWebglPostprocessingOutlineProjection();
        for (uint32_t entity = 0u; entity < RenderableCount; ++entity)
        {
            ...
            for (uint32_t uuidWord = 0u; uuidWord < 4u; ++uuidWord)
                (void)nextWebglPostprocessingOutlineRandom(randomState);
            glm::dvec3 direction(
                nextWebglPostprocessingOutlineRandom(randomState) - 0.5,
                nextWebglPostprocessingOutlineRandom(randomState) - 0.5,
                nextWebglPostprocessingOutlineRandom(randomState) - 0.5);
            direction = glm::normalize(direction);
            const glm::dvec3 position = direction *
                (nextWebglPostprocessingOutlineRandom(randomState) * 400.0);
            const glm::dvec3 rotation(
                nextWebglPostprocessingOutlineRandom(randomState) * 2.0,
                nextWebglPostprocessingOutlineRandom(randomState) * 2.0,
                nextWebglPostprocessingOutlineRandom(randomState) * 2.0);
            const double scale =
                nextWebglPostprocessingOutlineRandom(randomState) * 50.0;
            glm::dmat4 model = makeWebglPostprocessingOutlineRotation(
        ...
        for (uint32_t entity = 0u; entity < RenderableCount; ++entity)
        {
            ...
            const std::string vertexName =
                "WebglPostprocessingOutlineSphereVertices-" + entitySuffix;
            const std::string indexName =
                "WebglPostprocessingOutlineSphereIndices-" + entitySuffix;
            const std::string objectName =
                "WebglPostprocessingOutlineObject-" + entitySuffix;
            const std::string instanceName =
                "WebglPostprocessingOutlineInstance-" + entitySuffix;
            const std::string materialName =
                "WebglPostprocessingOutlineMaterial-" + entitySuffix;
            GVM::Core::RenderSetAllocInfo allocation;
            ...
            appendWebglPostprocessingOutlinePayload(
                allocation,
                WebglPostprocessingOutlineSceneRenderSetComponents::vertices,
                vertexName.c_str(),
                vertices.data(), vertices.size() * sizeof(vertices[0u]));
            appendWebglPostprocessingOutlinePayload(
                allocation,
                WebglPostprocessingOutlineSceneRenderSetComponents::indices,
                indexName.c_str(),
                indices.data(), indices.size() * sizeof(indices[0u]));
            appendWebglPostprocessingOutlinePayload(
                allocation,
                WebglPostprocessingOutlineSceneRenderSetComponents::objects,
                objectName.c_str(),
                &objects[entity], sizeof(objects[entity]));
            appendWebglPostprocessingOutlinePayload(
                allocation,
                WebglPostprocessingOutlineSceneRenderSetComponents::instances,
                instanceName.c_str(),
                &instanceData, sizeof(instanceData));
            appendWebglPostprocessingOutlinePayload(
                allocation,
                WebglPostprocessingOutlineSceneRenderSetComponents::materials,
                materialName.c_str(),
                &materialData, sizeof(materialData));
            appendWebglPostprocessingOutlinePayload(
                allocation,
                WebglPostprocessingOutlineSceneRenderSetComponents::renderFlags,
                "WebglPostprocessingOutlineRenderFlags",
    ...
    void WebglPostprocessingOutlineRuntimeAdapter::beforeFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex)
    {
    ...
    void WebglPostprocessingOutlineRuntimeAdapter::afterFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex,
        ...
        uint32_t height)
    {
        ...
        metadata
            ...
            << "\"caseId\":\"webgl_postprocessing_outline\",\"scenarioId\":\""
            ...
            << "\"gpuWorkDslOnly\":true}\n";
        writeWebglPostprocessingOutlineText(
        ...
        snapshot
            << "{\"schemaVersion\":1,\"caseId\":\"webgl_postprocessing_outline\","
            << "\"scenarioId\":\"" << options.scenarioId.c_str()
            << "\",\"frame\":" << frameIndex
            << ",\"implementationLevel\":\"semantic-complete\","
            << "\"gpuWorkDslOnly\":true,\"renderSetPolicy\":\"required\","
            << "\"sceneRenderSetCount\":1,\"renderableObjectCount\":23,"
            << "\"entityCount\":23,\"instanceCount\":23,"
            << "\"vertexCount\":" << vertices.size() * RenderableCount
            << ",\"indexCount\":" << indices.size() * RenderableCount
            << ",\"scenePassCount\":5,\"screenPassCount\":8,"
            << "\"drawCommandCount\":1,\"renderSetType\":"
            << "\"WebglPostprocessingOutlineSceneRenderSet\",\"sceneRoots\":[{"
            << "\"id\":\"scene\",\"renderSetCount\":1,"
            << "\"renderSetId\":\"scene-set\",\"renderSetType\":"
            << "\"WebglPostprocessingOutlineSceneRenderSet\","
            << "\"renderableObjectCount\":23,\"entityCount\":23,"
            ...
            << "\"WebglPostprocessingOutlineMainPass\",\"renderSetId\":\"scene-set\","
    ...
    void WebglPostprocessingOutlineRuntimeAdapter::shutdown(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options)
    {
        (void)renderer;
