...
{
    /** Mirrors one vertex in the spotlight Scene RenderSet ABI. */
    struct alignas(16) WebglLightsSpotlightsHostVertex
    {
        glm::vec4 position;
        glm::vec4 normal;
        glm::vec4 color;
    ...
    struct alignas(16) WebglLightsSpotlightsHostObjectData
    {
        glm::mat4 modelViewProjection;
        glm::mat4 modelView;
        glm::mat4 model;
        glm::vec4 positionAndKind;
        glm::vec4 light0PositionAngle;
        glm::vec4 light1PositionAngle;
        glm::vec4 light2PositionAngle;
        glm::vec4 lightPenumbra;
    };

    /** Mirrors the required one-entry instance component. */
    struct alignas(16) WebglLightsSpotlightsHostInstanceData
    {
        glm::vec4 reserved;
    };

    /** Mirrors one material tint and phase record. */
    struct alignas(16) WebglLightsSpotlightsHostMaterialData
    {
        glm::vec4 baseColor;
        glm::vec4 parameters;
    };

    /** Owns one floor, one box, and three helper entities before RenderSet allocation. */
    struct WebglLightsSpotlightsHostEntity
    {
        eastl::vector<WebglLightsSpotlightsHostVertex> vertices;
        eastl::vector<uint32_t> indices;
        WebglLightsSpotlightsHostObjectData objectData{};
        WebglLightsSpotlightsHostObjectData baseObjectData{};
        WebglLightsSpotlightsHostInstanceData instanceData{};
        WebglLightsSpotlightsHostMaterialData materialData{};
        GVM::Core::RenderEntityIndex entityIndex = UINT32_MAX;
    };

    /** Builds deterministic spotlight geometry and records its RGBA8 capture. */
    class WebglLightsSpotlightsRuntimeAdapter final
    {
    public:
        /** Generates the five RenderSet entities and configures fixed capture output. */
        ...
        /** Updates the three deterministic spotlight helper transforms. */
    ...
    private:
        ...
        eastl::vector<WebglLightsSpotlightsHostEntity> entities;
