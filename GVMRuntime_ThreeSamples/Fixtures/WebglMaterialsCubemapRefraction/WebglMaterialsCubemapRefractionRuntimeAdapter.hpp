...
{
    /** Mirrors one Lucy PLY triangle vertex in the RenderSet ABI. */
    struct alignas(16) WebglMaterialsCubemapRefractionHostVertex
    {
        glm::vec4 position;
        glm::vec4 normal;
    };

    /** Stores one refractive mesh transform, camera basis, and material mode. */
    struct alignas(16) WebglMaterialsCubemapRefractionHostObjectData
    {
    ...
    struct alignas(16) WebglMaterialsCubemapRefractionHostInstanceData
    {
        glm::vec4 reserved;
    };

    /** Stores the Three Phong tint, refraction ratio, and reflectivity. */
    struct alignas(16) WebglMaterialsCubemapRefractionHostMaterialData
    {
        glm::vec4 baseColor;
        glm::vec4 parameters;
    };

    /** Owns one Lucy entity, the six Park3Med cube faces, and its Set identity. */
    struct WebglMaterialsCubemapRefractionEntityData
    {
        eastl::vector<WebglMaterialsCubemapRefractionHostVertex> vertices;
        ...
        WebglMaterialsCubemapRefractionHostObjectData objectData{};
        WebglMaterialsCubemapRefractionHostObjectData baseObjectData{};
        WebglMaterialsCubemapRefractionHostInstanceData instanceData{};
        WebglMaterialsCubemapRefractionHostMaterialData materialData{};
        GVM::Core::RenderEntityIndex entityIndex = UINT32_MAX;
    };
    /** Drives the dedicated cubemap scene through one RenderSet. */
    class WebglMaterialsCubemapRefractionRuntimeAdapter final
    {
    public:
    ...
    private:
        ...
        /** Writes the canonical loader semantic sidecar for the locked Lucy asset. */
        void writeSemanticSnapshot(const ThreeSampleHostOptions &,uint32_t) const;
        GVM::Core::DeviceProxy device; eastl::vector<WebglMaterialsCubemapRefractionEntityData> entities; bool captureWritten=false;
    };
} // namespace GVM::ThreeSamples
