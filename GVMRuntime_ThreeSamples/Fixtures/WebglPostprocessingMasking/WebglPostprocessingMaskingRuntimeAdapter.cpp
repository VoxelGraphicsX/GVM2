#include "WebglPostprocessingMaskingRuntimeAdapter.hpp"
...
namespace GVM::ThreeSamples
{
    namespace
    {
        constexpr GVM::Core::RenderSetHandle Scene1SetHandle =
            ExportedRenderSet::scene1Set;
        constexpr double Pi = 3.14159265358979323846;

        static_assert(sizeof(WebglPostprocessingMaskingVertex) == 16u);
        static_assert(sizeof(WebglPostprocessingMaskingTorusUniforms) == 64u);
        static_assert(sizeof(WebglPostprocessingMaskingHostObjectData) == 64u);
        static_assert(sizeof(WebglPostprocessingMaskingHostInstanceData) == 16u);
        static_assert(sizeof(WebglPostprocessingMaskingHostMaterialData) == 16u);

        /** Creates parent directories for one requested evidence artifact. */
        void prepareWebglMaskingOutput(const std::filesystem::path &path)
        {
        ...
        void writeWebglMaskingText(
            const eastl::string &path,
            const std::string &text)
        {
            if (path.empty()) return;
            const std::filesystem::path outputPath(path.c_str());
            prepareWebglMaskingOutput(outputPath);
            std::ofstream output(outputPath, std::ios::trunc);
            output << text;
            if (!output)
                throw std::runtime_error(
                    "Could not write WebGL masking evidence.");
        }

        /** Flips decoded browser image rows for Texture.flipY=true upload. */
        RgbaImageData flipWebglMaskingRows(const RgbaImageData &source)
        {
        ...
        eastl::vector<eastl::vector<uint8_t>> packWebglMaskingMips(
        ...
        glm::dmat4 makeWebglMaskingRotation(
            double rotationX,
            double rotationY)
        {
        ...
        /** Constructs the fixed 50-degree WebGL camera projection. */
        glm::dmat4 makeWebglMaskingProjection()
        {
        ...
        glm::mat4 makeWebglMaskingMvp(
            double positionX,
            double positionY,
            double rotationX,
            double rotationY)
        {
            glm::dmat4 model = makeWebglMaskingRotation(
                rotationX, rotationY);
            model[3u] = glm::dvec4(positionX, positionY, 0.0, 1.0);
            glm::dmat4 view(1.0);
            view[3u][2u] = -10.0;
            return glm::mat4(makeWebglMaskingProjection() * view * model);
        }

        /** Builds a grouped BoxGeometry-equivalent closed cube. */
        void buildWebglMaskingBox(
            eastl::vector<WebglPostprocessingMaskingVertex> &vertices,
            ...
            eastl::vector<uint32_t> &indices)
        {
            ...
            if (vertices.size() != 561u || indices.size() != 3072u)
                throw std::runtime_error(
                    "WebGL masking torus topology differs from r185.");
        }

        /** Appends one typed payload to the grouped box allocation. */
        void appendWebglMaskingPayload(
    ...
    void WebglPostprocessingMaskingRuntimeAdapter::initializeResources(
        ...
        const ThreeSampleHostOptions &options)
    {
        ...
        if (options.caseId != "webgl_postprocessing_masking" ||
            (!initial && !animated) ||
            ...
            options.assetRoot.empty() || !options.inputReplayPath.empty())
        {
            throw std::invalid_argument(
                "WebGL masking requires its locked scenarios, extent, seed, assets, and no replay.");
        }
        device = inDevice;
        const double time = 6000.0 + double(options.targetFrame) / 60.0;
        buildWebglMaskingBox(boxVertices, boxIndices);
        buildWebglMaskingTorus(torusVertices, torusIndices);
        boxObjectData.modelViewProjection = makeWebglMaskingMvp(
        ...
        torusUniforms.modelViewProjection = makeWebglMaskingMvp(
            std::cos(time) * 2.0,
        ...
        const RgbaImageData decoded1 = flipWebglMaskingRows(
            decodeJpegRgba8(
                textureRoot /
                "758px-Canestra_di_frutta_(Caravaggio).jpg"));
        const RgbaImageData decoded2 = flipWebglMaskingRows(
            decodeJpegRgba8(
        ...
        if (decoded1.width != 758u || decoded1.height != 600u ||
            ...
            throw std::runtime_error(
                "WebGL masking photograph dimensions differ from r185.");
        photograph1Mips.push_back(decoded1.pixels);
        photograph2Mips = packWebglMaskingMips(
        ...
        appendWebglMaskingPayload(
            allocation,
            WebglPostprocessingMaskingScene1RenderSetComponents::vertices,
            "WebglPostprocessingMaskingBoxVertices",
            boxVertices.data(),
            boxVertices.size() * sizeof(WebglPostprocessingMaskingVertex));
        appendWebglMaskingPayload(
            allocation,
            WebglPostprocessingMaskingScene1RenderSetComponents::indices,
            "WebglPostprocessingMaskingBoxIndices",
            boxIndices.data(),
            boxIndices.size() * sizeof(uint32_t));
        appendWebglMaskingPayload(
            allocation,
            WebglPostprocessingMaskingScene1RenderSetComponents::objects,
            "WebglPostprocessingMaskingBoxObject",
            &boxObjectData,
            sizeof(boxObjectData));
        appendWebglMaskingPayload(
            allocation,
            WebglPostprocessingMaskingScene1RenderSetComponents::instances,
            "WebglPostprocessingMaskingBoxInstance",
            &boxInstanceData,
            sizeof(boxInstanceData));
        appendWebglMaskingPayload(
            allocation,
            WebglPostprocessingMaskingScene1RenderSetComponents::materials,
            "WebglPostprocessingMaskingBoxMaterial",
            &boxMaterialData,
        ...
        if (!encoder)
            throw std::runtime_error(
                "Could not create the WebGL masking Scene1 Set encoder.");
        encoder->allocEntity(allocation);
        renderer.executeRenderSetCommand(Scene1SetHandle, encoder);
    }

    void WebglPostprocessingMaskingRuntimeAdapter::beforeFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex)
    {
    ...
    void WebglPostprocessingMaskingRuntimeAdapter::afterFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex,
        ...
        uint32_t height)
    {
        ...
        if (!options.captureRgbaPath.empty())
        {
            ...
            if (!output)
                throw std::runtime_error(
                    "Could not write WebGL masking RGBA.");
        }
        ...
        metadata
            ...
            << ",\"byteCount\":" << byteCount
            << ",\"format\":\"rgba8unorm\"}\n";
        writeWebglMaskingText(options.captureMetadataPath, metadata.str());
        ...
        snapshot
            ...
            << ",\"scenePassCount\":2,\"screenPassCount\":4,"
            << "\"drawCommandCount\":2,\"renderSetType\":"
            << "\"WebglPostprocessingMaskingScene1RenderSet\","
            << "\"sceneRoots\":[{\"id\":\"scene1\","
            << "\"renderSetCount\":1,\"renderSetId\":\"scene1-set\","
            << "\"renderSetType\":\"WebglPostprocessingMaskingScene1RenderSet\","
            ...
            << "\"scenePasses\":[{\"name\":\"scene1-mask\","
            << "\"renderClass\":\"WebglPostprocessingMaskingScene1MaskPass\","
            << "\"renderSetId\":\"scene1-set\",\"renderSetBindingCount\":1,"
            << "\"drawMode\":\"render-set-indexed-indirect\","
            << "\"invocationCount\":1,\"drawCommandCount\":1,"
            << "\"usesStandaloneGeometry\":false,"
            << "\"usesExplicitDrawCount\":false}]},{\"id\":\"scene2\","
            << "\"renderSetCount\":0,\"renderableObjectCount\":1}],"
            << "\"scenePassSequence\":["
            << "{\"sceneRoot\":\"scene1\",\"scenePass\":\"scene1-mask\","
            << "\"entityOrdinal\":0},{\"sceneRoot\":\"scene2\","
            << "\"scenePass\":\"scene2-mask\",\"entityOrdinal\":0}]}\n";
        writeWebglMaskingText(options.sceneSnapshotPath, snapshot.str());
        writeWebglMaskingText(options.semanticSnapshotPath, snapshot.str());
        captureWritten = true;
    }

    void WebglPostprocessingMaskingRuntimeAdapter::shutdown(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options)
    {
        (void)renderer;
