...
namespace GVM::ThreeSamples
{
    ...
    struct alignas(16) WebgpuRttHostVertex final
    {
        glm::vec4 position;
    ...
    struct alignas(16) WebgpuRttHostObjectData final
    {
        glm::mat4 modelViewProjection;
    };

    /** Mirrors the mandatory ordinary-entity instance component. */
    struct alignas(16) WebgpuRttHostInstanceData final
    {
        glm::vec4 reserved;
    };

    /** Mirrors the white MeshBasic color multiplier. */
    struct alignas(16) WebgpuRttHostMaterialData final
    {
        glm::vec4 baseColor;
    };

    /** Connects exact r185 RTT state to the dedicated generated renderer. */
    class WebgpuRttRuntimeAdapter final
    {
    public:
    ...
    private:
        ...
        eastl::vector<WebgpuRttHostVertex> vertices;
        eastl::vector<uint32_t> indices;
        eastl::vector<uint8_t> textureBytes;
        eastl::vector<uint64_t> textureMipOffsets;
        WebgpuRttHostObjectData objectData{};
        WebgpuRttHostInstanceData instanceData{};
        WebgpuRttHostMaterialData materialData{};
        float pointerX = 0.0f;
        float pointerY = 0.0f;
