#include "WebglInteractiveCubesRuntimeAdapter.hpp"
...
namespace GVM::ThreeSamples
{
    namespace
    {
        ...
        void prepareCubesPath(const std::filesystem::path &path)
        {
        ...
        float cubesSrgbByteToLinear(uint32_t value)
        {
        ...
        /** Advances the deterministic xorshift stream used in the upstream loop. */
        uint32_t nextCubeRandom(uint32_t &state)
        {
            state ^= state << 13u;
            state ^= state >> 17u;
            state ^= state << 5u;
        ...
        /** Returns a deterministic unit interval sample from the xorshift stream. */
        float cubeRandom(uint32_t &state)
        {
            return float(nextCubeRandom(state) >> 8u) / 16777216.0f;
        }

        /** Appends one typed payload to a RenderSet allocation. */
        void appendCubesBuffer(
        ...
        bool intersectCubeRay(const glm::vec3 &origin,
        ...
        uint32_t findCenterPointerCube(const eastl::vector<WebglInteractiveCubesEntityData> &entities,
                                       const glm::mat4 &view,
                                       const glm::mat4 &projection)
        {
            ...
            {
                float distance = 0.0f;
                if (intersectCubeRay(origin, direction, entities[index].model, distance) &&
        ...
        /** Builds a face-separated unit BoxGeometry with stable normals. */
        void buildCubeGeometry(
            eastl::vector<WebglInteractiveCubesHostVertex> &vertices,
        ...
        /** Returns the WebGL perspective projection used by the example. */
        glm::mat4 makeCubesProjection(uint32_t width, uint32_t height)
        {
        ...
        /** Validates the frozen initial, animated, and pointer-hit scenarios. */
        void validateCubesOptions(const ThreeSampleHostOptions &options)
        {
            ...
            if (options.caseId != "webgl_interactive_cubes" ||
                (!initial && !animated && !hit) ||
                options.width != 800u || options.height != 500u ||
                options.randomSeed != DefaultThreeRandomSeed)
            {
                throw std::invalid_argument(
                    "webgl_interactive_cubes scenario does not match the locked r185 contract.");
            }
        }
    } // namespace

    void WebglInteractiveCubesRuntimeAdapter::initializeResources(
        GVM::Core::AbstractRendererImpl &renderer,
        GVM::Core::DeviceProxy inDevice,
        const ThreeSampleHostOptions &options)
    {
        validateCubesOptions(options);
        device = inDevice;
        ...
        // Scene/camera/light/geometry construction consumes the first UUID
        // samples before the example's cube loop begins.
        for (uint32_t sample = 0u; sample < InitialThreeUuidRandomSamples; ++sample)
        {
            (void)cubeRandom(randomState);
        }
        for (uint32_t entityIndex = 0u; entityIndex < CubeCount; ++entityIndex)
        {
            WebglInteractiveCubesEntityData &entity = entities[entityIndex];
            buildCubeGeometry(entity.vertices, entity.indices);
            // The r185 example consumes Math.random() for the material before
            // position, rotation, and scale; preserving that order is required
            // for the locked seeded replay to produce the same cube field.
            const uint32_t packedColor = static_cast<uint32_t>(cubeRandom(randomState) * 0x00ffffffu);
            for (uint32_t sample = 0u; sample < PerCubeThreeUuidRandomSamples; ++sample)
            {
                (void)cubeRandom(randomState);
            }
            const glm::vec3 position(
                cubeRandom(randomState) * 40.0f - 20.0f,
                cubeRandom(randomState) * 40.0f - 20.0f,
                cubeRandom(randomState) * 40.0f - 20.0f);
            const glm::vec3 rotation(
                cubeRandom(randomState) * float(2.0 * Pi),
                cubeRandom(randomState) * float(2.0 * Pi),
                cubeRandom(randomState) * float(2.0 * Pi));
            const glm::vec3 scale(
                cubeRandom(randomState) + 0.5f,
                cubeRandom(randomState) + 0.5f,
                cubeRandom(randomState) + 0.5f);
            ...
            entity.objectData.modelViewProjection = entity.model;
            entity.objectData.modelView = entity.model;
            ...
            entity.materialData.baseColor = glm::vec4(
                cubesSrgbByteToLinear((packedColor >> 16u) & 0xffu),
                cubesSrgbByteToLinear((packedColor >> 8u) & 0xffu),
                cubesSrgbByteToLinear(packedColor & 0xffu),
                1.0f);
        ...
        if (!encoder)
        {
            throw std::runtime_error(
                "webgl_interactive_cubes could not create its Scene Set encoder.");
        }
        for (uint32_t entityIndex = 0u; entityIndex < CubeCount; ++entityIndex)
        {
            WebglInteractiveCubesEntityData &entity = entities[entityIndex];
            ...
            const eastl::string prefix =
                eastl::string("InteractiveCube-") + eastl::to_string(entityIndex);
            appendCubesBuffer(
                allocation,
                WebglInteractiveCubesSceneRenderSetComponents::vertices,
                prefix + "-vertices",
                entity.vertices.data(),
                entity.vertices.size() * sizeof(WebglInteractiveCubesHostVertex),
                1u);
            appendCubesBuffer(
                allocation,
                WebglInteractiveCubesSceneRenderSetComponents::indices,
                prefix + "-indices",
                entity.indices.data(),
            ...
            appendCubesBuffer(
                allocation,
                WebglInteractiveCubesSceneRenderSetComponents::objects,
                prefix + "-object",
                &entity.objectData,
            ...
            appendCubesBuffer(
                allocation,
                WebglInteractiveCubesSceneRenderSetComponents::instances,
                prefix + "-instance",
                &entity.instanceData,
            ...
            appendCubesBuffer(
                allocation,
                WebglInteractiveCubesSceneRenderSetComponents::materials,
                prefix + "-material",
                &entity.materialData,
    ...
    void WebglInteractiveCubesRuntimeAdapter::updateObjectData(
        uint32_t width,
        uint32_t height,
        uint32_t frameIndex)
    {
        const double thetaDegrees = 0.1 + 0.1 * double(frameIndex);
        ...
        const glm::mat4 projection = makeCubesProjection(width, height);
        currentViewMatrix = view;
        currentProjectionMatrix = projection;
        // Three stores the DirectionalLight in world space; the fragment
        // shader consumes view-space normals, so transform the light for each
        // camera pose instead of keeping a fixed view-space direction.
        const glm::vec3 viewLightDirection = glm::normalize(
            glm::mat3(view) * glm::normalize(glm::vec3(1.0f, 1.0f, 1.0f)));
        for (WebglInteractiveCubesEntityData &entity : entities)
        {
    ...
    void WebglInteractiveCubesRuntimeAdapter::beforeFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex)
    {
        ...
        selectedCubeIndex = options.scenarioId == "cube-hit"
            ? findCenterPointerCube(entities, currentViewMatrix, currentProjectionMatrix)
            : UINT32_MAX;
        const auto encoder = renderer.createRenderSetCommandEncoder(SceneRenderSetHandle);
        if (!encoder)
        {
            throw std::runtime_error(
                "webgl_interactive_cubes could not create its update encoder.");
        }
        const bool highlight = options.scenarioId == "cube-hit";
        for (const WebglInteractiveCubesEntityData &entity : entities)
        {
        ...
        if (highlight && selectedCubeIndex < entities.size())
        {
            WebglInteractiveCubesHostMaterialData hitMaterial = entities[selectedCubeIndex].materialData;
            hitMaterial.emissive = glm::vec4(1.0f, 0.0f, 0.0f, 1.0f);
            encoder->setBufferComponentData(
                entities[selectedCubeIndex].entityIndex,
                WebglInteractiveCubesSceneRenderSetComponents::materials,
                &hitMaterial,
                sizeof(hitMaterial),
                0u,
    ...
    void WebglInteractiveCubesRuntimeAdapter::writeRgbaCapture(
        const ThreeSampleHostOptions &options,
        const eastl::vector<uint8_t> &rgba) const
    {
        if (options.captureRgbaPath.empty()) return;
        const std::filesystem::path path(options.captureRgbaPath.c_str());
        prepareCubesPath(path);
        std::ofstream output(path, std::ios::binary | std::ios::trunc);
        output.write(reinterpret_cast<const char *>(rgba.data()),
                     static_cast<std::streamsize>(rgba.size()));
        if (!output)
        {
            throw std::runtime_error("webgl_interactive_cubes could not write RGBA capture.");
        }
    }

    void WebglInteractiveCubesRuntimeAdapter::writeCaptureMetadata(
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex,
        uint32_t width,
        uint32_t height,
        uint64_t byteCount) const
    {
        ...
        output << "{\"schemaVersion\":1,\"source\":\"gvm-three-r185\","
               << "\"caseId\":\"webgl_interactive_cubes\",\"scenarioId\":\""
    ...
    void WebglInteractiveCubesRuntimeAdapter::writeStructuralSnapshot(
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex) const
    {
        ...
        output << "{\n"
               << "  \"schemaVersion\":1,\n"
               << "  \"caseId\":\"webgl_interactive_cubes\",\n"
               ...
               << "  \"assetAndAlgorithmState\":\"cpu-boxgeometry-xorshift-random-lambert\",\n"
               << "  \"renderSetType\":\"WebglInteractiveCubesSceneRenderSet\",\n"
               << "  \"scenePasses\":[{\"name\":\"main-lambert\",\"renderClass\":\"WebglInteractiveCubesScenePass\",\"renderSetId\":\"scene-set-0\",\"renderSetBindingCount\":1,\"drawMode\":\"render-set-indexed-indirect\",\"invocationCount\":1,\"drawCommandCount\":1,\"usesStandaloneGeometry\":false,\"usesExplicitDrawCount\":false}],\n"
               << "  \"sceneRoots\":[{\"id\":\"scene\",\"renderSetCount\":1,\"renderSetType\":\"WebglInteractiveCubesSceneRenderSet\",\"renderableObjectCount\":2000,\"entityCount\":2000,\"drawCommandCount\":1,\"directDrawFallback\":false,\"scenePasses\":[\"WebglInteractiveCubesScenePass\"],\"entities\":[";
        for (uint32_t index = 0u; index < CubeCount; ++index)
        {
            if (index != 0u) output << ',';
            output << "{\"entityId\":" << index
                   << ",\"logicalRenderableId\":\"cube-" << index
                   << "\",\"instanceCount\":1}";
        }
        output << "]}]}\n";
    }

    void WebglInteractiveCubesRuntimeAdapter::afterFrame(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options,
        uint32_t frameIndex,
        ...
        uint32_t height)
    {
        ...
        {
            throw std::overflow_error("webgl_interactive_cubes RGBA capture is too large.");
        }
        eastl::vector<uint8_t> rgba(static_cast<size_t>(byteCount));
        const auto queue = device->graphicsQueue(0);
        if (!queue)
        {
            throw std::runtime_error("webgl_interactive_cubes could not access graphics queue.");
        }
    ...
    void WebglInteractiveCubesRuntimeAdapter::shutdown(
        GVM::Core::AbstractRendererImpl &renderer,
        const ThreeSampleHostOptions &options)
    {
        (void)renderer;
