...
struct WebgpuCameraArrayVertex
{
...
struct WebgpuCameraArrayMaterialData
{
    float4 baseColorAndPhase;
};

/** Stores the required identity instance component for non-instanced entities. */
struct WebgpuCameraArrayInstanceData
{
    float4 identity;
};

/** Stores the required object component and the receive-shadow flag. */
struct WebgpuCameraArrayObjectData
{
    float4 receiveShadowAndReserved;
};

/** Defines the unique two-entity Scene RenderSet for this array-camera example. */
struct WebgpuCameraArraySceneRenderSet : public IRenderSet
{
    /** Declares consolidated geometry and mandatory object, instance, and material components. */
    constructor(
        BufferComponent<WebgpuCameraArrayVertex> vertices [[RenderSetVertexBuffer]],
        BufferComponent<uint> indices [[RenderSetIndexBuffer]],
        BufferComponent<WebgpuCameraArrayObjectData> objects,
        BufferComponent<WebgpuCameraArrayInstanceData> instances,
        BufferComponent<WebgpuCameraArrayMaterialData> materials)
    {
    }
};

/** Binds the current directional shadow depth texture. */
struct WebgpuCameraArrayMainResources final : public IBindGroup
{
...
struct WebgpuCameraArrayShadowVertexOutput
{
...
struct WebgpuCameraArrayShadowFrameBuffer final : public IFrameBuffer
{
    DepthStencilAttachment<TextureFormat::Depth32Float> depth;
};

/** Carries projected tile, world-space lighting, and entity data into the main fragment stage. */
struct WebgpuCameraArrayMainVertexOutput
{
    float4 position [[Position]];
...
struct WebgpuCameraArrayMainFrameBuffer final : public IFrameBuffer
{
...
float WebgpuCameraArraylinearToSrgb(float value)
{
...
float WebgpuCameraArrayreadShadowDepth(
    IN BindGroup<WebgpuCameraArrayMainResources> resources,
    uint2 texel)
{
...
float WebgpuCameraArraybilinearShadowCompare(
    IN BindGroup<WebgpuCameraArrayMainResources> resources,
    float2 uv,
    float compareDepth)
{
    ...
    const float compare00 = compareDepth <= WebgpuCameraArrayreadShadowDepth(resources, uint2(x0, y0)) ? 1.0f : 0.0f;
    const float compare10 = compareDepth <= WebgpuCameraArrayreadShadowDepth(resources, uint2(x1, y0)) ? 1.0f : 0.0f;
    const float compare01 = compareDepth <= WebgpuCameraArrayreadShadowDepth(resources, uint2(x0, y1)) ? 1.0f : 0.0f;
    const float compare11 = compareDepth <= WebgpuCameraArrayreadShadowDepth(resources, uint2(x1, y1)) ? 1.0f : 0.0f;
    return lerp(
...
float WebgpuCameraArrayshadowFactor(
    IN BindGroup<WebgpuCameraArrayMainResources> resources,
    float3 lightUvDepth,
    float2 screenPosition)
{
    ...
    for (uint sampleIndex = 0u; sampleIndex < 5u; ++sampleIndex)
    {
        ...
        shadow += WebgpuCameraArraybilinearShadowCompare(
            resources,
...
class WebgpuCameraArrayShadowDepthPass final : public IRenderClass
{
public:
    /** Binds the same unique Scene RenderSet used by every main invocation. */
    constructor(RenderSet<WebgpuCameraArraySceneRenderSet> sceneSet [[Slot0]])
    {
...
private:
    ...
    WebgpuCameraArrayShadowVertexOutput vertex(
        WebgpuCameraArrayVertex inputValue [[VertexInput0]],
        uint renderEntityID [[RenderEntityID]],
        uint renderEntityInstanceID [[RenderEntityInstanceID]])
    {
        const WebgpuCameraArrayInstanceData instanceData =
            sceneSet->instances->get(renderEntityID, renderEntityInstanceID);
        WebgpuCameraArrayShadowVertexOutput outputValue;
    ...
    WebgpuCameraArrayShadowFrameBuffer fragment(
        WebgpuCameraArrayShadowVertexOutput inputValue)
    {
        if (inputValue.entityID == 0u)
        {
            discard_fragment();
        }
        WebgpuCameraArrayShadowFrameBuffer frameBuffer;
        return frameBuffer;
    }
};

/** Draws the two entities across their preprojected six-by-six camera tiles. */
class WebgpuCameraArrayMainPass final : public IRenderClass
{
public:
    /** Binds exactly one Scene RenderSet plus the directional depth texture. */
    constructor(
        RenderSet<WebgpuCameraArraySceneRenderSet> sceneSet [[Slot0]],
        BindGroup<WebgpuCameraArrayMainResources> resources [[Slot1]])
    {
        setCullMode(CullMode::None);
        setDepthWriteEnabled(true);
        setDepthCompareFunction(CompareFunction::LessEqual);
...
private:
    ...
    WebgpuCameraArrayMainVertexOutput vertex(
        WebgpuCameraArrayVertex inputValue [[VertexInput0]],
        uint renderEntityID [[RenderEntityID]],
        uint renderEntityInstanceID [[RenderEntityInstanceID]])
    {
        const WebgpuCameraArrayObjectData objectData =
            sceneSet->objects->get(renderEntityID, 0u);
        const WebgpuCameraArrayInstanceData instanceData =
            sceneSet->instances->get(renderEntityID, renderEntityInstanceID);
        WebgpuCameraArrayMainVertexOutput outputValue;
    ...
    WebgpuCameraArrayMainFrameBuffer fragment(
        WebgpuCameraArrayMainVertexOutput inputValue)
    {
        ...
        const WebgpuCameraArrayMaterialData materialData =
        ...
        const float3 cameraPosition = float3(
            ...
            1.0f - float(tileY) / 3.0f,
            3.0f + float(tileX + tileY));
        ...
        const float3 encoded = float3(
            WebgpuCameraArraylinearToSrgb(linearColor.x),
            WebgpuCameraArraylinearToSrgb(linearColor.y),
            WebgpuCameraArraylinearToSrgb(linearColor.z));
        WebgpuCameraArrayMainFrameBuffer frameBuffer;
...
class WebgpuCameraArrayRenderer final : public AbstractRenderer
{
    Device device;
    Swapchain swapchain;
    Queue graphicsQueue;
    [[Export]] RenderSet<WebgpuCameraArraySceneRenderSet> sceneSet;
    ...
    BindGroup<WebgpuCameraArrayMainResources> mainResources;
    RenderClass<WebgpuCameraArrayShadowDepthPass> shadowPass;
    RenderClass<WebgpuCameraArrayMainPass> mainPass;
    uint width = 800u;
    uint height = 500u;
...
public:
    ...
    void init(Device inDevice, Swapchain inSwapchain) override
    {
        ...
        sceneSet = device->createRenderSet<WebgpuCameraArraySceneRenderSet>();
        shadowDepth = device->createTexture(
            ...
            1u);
        mainResources =
            device->createBindGroup<WebgpuCameraArrayMainResources>(
                shadowDepth->createView());
        shadowPass =
            device->createRenderClass<WebgpuCameraArrayShadowDepthPass>(
                sceneSet);
        mainPass =
            device->createRenderClass<WebgpuCameraArrayMainPass>(
                sceneSet,
                mainResources);
    }
    ...
    {
        sceneSet->update();
        WebgpuCameraArrayShadowFrameBuffer shadowFrameBuffer;
        ...
        WebgpuCameraArrayMainFrameBuffer mainFrameBuffer;
