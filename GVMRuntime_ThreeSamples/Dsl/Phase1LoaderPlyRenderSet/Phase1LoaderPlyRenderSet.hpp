#ifndef GVM_THREE_PHASE1_LOADER_PLY_RENDER_SET_HPP
#define GVM_THREE_PHASE1_LOADER_PLY_RENDER_SET_HPP

#include "UGL.h"

using namespace UGL;

static const uint WebglLoaderPlyShadowMapSize = 1024u;

/** Stores one position and generated normal from the consolidated PLY scene geometry. */
struct WebglLoaderPlyVertex
{
...
struct WebglLoaderPlyObjectData
{
    float4x4 model;
...
struct WebglLoaderPlyInstanceData
{
    float4 translation;
};

/** Stores the linear base color and Phong or Standard specular parameters for one entity. */
struct WebglLoaderPlyMaterialData
{
    float4 baseColor;
    float4 specularAndSurfaceParameter;
};

/** Stores material kind, cast-shadow, receive-shadow, and logical entity phase flags. */
struct WebglLoaderPlyRenderFlags
{
    uint4 values;
};

/** Defines the unique RenderSet shared by every geometry pass for the PLY Scene. */
struct WebglLoaderPlySceneRenderSet : public IRenderSet
{
    /** Declares the single vertex/index stores and all per-entity Scene components. */
    constructor(BufferComponent<WebglLoaderPlyVertex> vertices [[RenderSetVertexBuffer]], BufferComponent<uint> indices [[RenderSetIndexBuffer]], BufferComponent<WebglLoaderPlyObjectData> objects, BufferComponent<WebglLoaderPlyInstanceData> instances, BufferComponent<WebglLoaderPlyMaterialData> materials, BufferComponent<WebglLoaderPlyRenderFlags> renderFlags)
    {
    }
};

/** Selects one of the two frozen directional-light matrices for a shadow invocation. */
struct WebglLoaderPlyShadowInvocationData
{
    uint4 lightIndexAndReserved;
};

/** Binds only the shadow invocation selector in addition to the unique Scene RenderSet. */
struct WebglLoaderPlyShadowInvocationBindGroup final : public IBindGroup
{
    /** Declares the immutable per-invocation selector uniform. */
    constructor(UniformBuffer<WebglLoaderPlyShadowInvocationData> invocation [[Binding0]])
    {
    }
};

/** Binds the two depth textures produced by the Scene shadow invocations. */
struct WebglLoaderPlyLitResources final : public IBindGroup
{
...
struct WebglLoaderPlyShadowVertexOutput
{
...
struct WebglLoaderPlyShadowFrameBuffer final : public IFrameBuffer
{
    DepthStencilAttachment<TextureFormat::Depth32Float> depth;
};

/** Carries world-space lighting, camera, shadow, material, and phase inputs. */
struct WebglLoaderPlyLitVertexOutput
{
    float4 position [[Position]];
    ...
    float2 fogRange [[Attribute6]];
    uint2 entityAndPackedPhase [[Attribute7]];
};

/** Defines the single-sample color and depth framebuffer for the main Scene invocation. */
struct WebglLoaderPlyLitFrameBuffer final : public IFrameBuffer
{
...
float webglLoaderPlyLinearToSrgb(float value)
{
    if (value <= 0.0031308f)
    {
        return value * 12.92f;
...
float3 webglLoaderPlyFresnelSchlick(float3 f0, float f90, float dotViewHalf)
{
...
float2 webglLoaderPlyDfgRoughnessOneRow(uint row)
{
    if (row == 0u)
...
float2 webglLoaderPlyDfgRoughnessOne(float dotNormalView)
{
    const float texelCoordinate = clamp(dotNormalView, 0.0f, 1.0f) * 16.0f - 0.5f;
    if (texelCoordinate <= 0.0f)
        return webglLoaderPlyDfgRoughnessOneRow(0u);
    if (texelCoordinate >= 15.0f)
        return webglLoaderPlyDfgRoughnessOneRow(15u);
    const float lowerCoordinate = floor(texelCoordinate);
    const uint lowerRow = uint(lowerCoordinate);
    return lerp(webglLoaderPlyDfgRoughnessOneRow(lowerRow), webglLoaderPlyDfgRoughnessOneRow(lowerRow + 1u), texelCoordinate - lowerCoordinate);
}

/** Evaluates Three r185's roughness-one GGX BRDF including its DFG multiscatter term. */
float3 webglLoaderPlyPhysicalSpecular(float3 lightDirection, float3 viewDirection, float3 normal)
{
    ...
    const float3 fresnel = webglLoaderPlyFresnelSchlick(f0, 1.0f, dotViewHalf);
    ...
    const float2 dfgView = webglLoaderPlyDfgRoughnessOne(dotNormalView);
    const float2 dfgLight = webglLoaderPlyDfgRoughnessOne(dotNormalLight);
...
float3 webglLoaderPlyPhongSpecular(float3 lightDirection, float3 viewDirection, float3 normal, float3 specularColor, float shininess)
{
    const float3 halfDirection = normalize(lightDirection + viewDirection);
    const float dotNormalHalf = clamp(dot(normal, halfDirection), 0.0f, 1.0f);
    const float dotViewHalf = clamp(dot(viewDirection, halfDirection), 0.0f, 1.0f);
    const float3 fresnel = webglLoaderPlyFresnelSchlick(specularColor, 1.0f, dotViewHalf);
...
float webglLoaderPlyReadShadowDepth(IN BindGroup<WebglLoaderPlyLitResources> resources, uint lightIndex, uint2 texel)
{
    if (lightIndex == 0u)
    {
...
float webglLoaderPlyBilinearShadowCompare(IN BindGroup<WebglLoaderPlyLitResources> resources, uint lightIndex, float2 uv, float compareDepth)
{
    const float2 texelPosition = uv * float(WebglLoaderPlyShadowMapSize) - 0.5f;
    const float2 lowerPosition = floor(texelPosition);
    const float2 fraction = texelPosition - lowerPosition;
    const uint x0 = uint(clamp(lowerPosition.x, 0.0f, float(WebglLoaderPlyShadowMapSize - 1u)));
    const uint y0 = uint(clamp(lowerPosition.y, 0.0f, float(WebglLoaderPlyShadowMapSize - 1u)));
    const uint x1 = min(x0 + 1u, WebglLoaderPlyShadowMapSize - 1u);
    const uint y1 = min(y0 + 1u, WebglLoaderPlyShadowMapSize - 1u);
    const float compare00 = compareDepth <= webglLoaderPlyReadShadowDepth(resources, lightIndex, uint2(x0, y0)) ? 1.0f : 0.0f;
    const float compare10 = compareDepth <= webglLoaderPlyReadShadowDepth(resources, lightIndex, uint2(x1, y0)) ? 1.0f : 0.0f;
    const float compare01 = compareDepth <= webglLoaderPlyReadShadowDepth(resources, lightIndex, uint2(x0, y1)) ? 1.0f : 0.0f;
    const float compare11 = compareDepth <= webglLoaderPlyReadShadowDepth(resources, lightIndex, uint2(x1, y1)) ? 1.0f : 0.0f;
...
float2 webglLoaderPlyVogelDiskSample(uint sampleIndex, float phase)
{
...
float webglLoaderPlyDirectionalShadow(IN BindGroup<WebglLoaderPlyLitResources> resources, uint lightIndex, float4 shadowClip, float2 finalPixelPosition)
{
    ...
    const float radius = 1.0f / float(WebglLoaderPlyShadowMapSize);
    const float compareDepth = shadowCoordinate.z - 0.001f;
    return (webglLoaderPlyBilinearShadowCompare(resources, lightIndex, shadowCoordinate.xy + webglLoaderPlyVogelDiskSample(0u, phase) * radius, compareDepth) + webglLoaderPlyBilinearShadowCompare(resources, lightIndex, shadowCoordinate.xy + webglLoaderPlyVogelDiskSample(1u, phase) * radius, compareDepth) + webglLoaderPlyBilinearShadowCompare(resources, lightIndex, shadowCoordinate.xy + webglLoaderPlyVogelDiskSample(2u, phase) * radius, compareDepth) +
            webglLoaderPlyBilinearShadowCompare(resources, lightIndex, shadowCoordinate.xy + webglLoaderPlyVogelDiskSample(3u, phase) * radius, compareDepth) + webglLoaderPlyBilinearShadowCompare(resources, lightIndex, shadowCoordinate.xy + webglLoaderPlyVogelDiskSample(4u, phase) * radius, compareDepth)) *
           0.2f;
}

/** Draws one directional shadow invocation through the Scene's unique RenderSet. */
class WebglLoaderPlyShadowDepthPass final : public IRenderClass
{
public:
    /** Binds the unique Scene RenderSet and one immutable directional-light selector. */
    constructor(RenderSet<WebglLoaderPlySceneRenderSet> sceneSet [[Slot0]], BindGroup<WebglLoaderPlyShadowInvocationBindGroup> invocationResources [[Slot1]])
    {
...
private:
    ...
    WebglLoaderPlyShadowVertexOutput vertex(WebglLoaderPlyVertex inputValue [[VertexInput0]], uint renderEntityID [[RenderEntityID]])
    {
        const WebglLoaderPlyObjectData objectData = sceneSet->objects->get(renderEntityID, 0u);
        const WebglLoaderPlyInstanceData instanceData = sceneSet->instances->get(renderEntityID, 0u);
        const WebglLoaderPlyRenderFlags flags = sceneSet->renderFlags->get(renderEntityID, 0u);
        ...
        WebglLoaderPlyShadowVertexOutput outputValue;
    ...
    WebglLoaderPlyShadowFrameBuffer fragment(WebglLoaderPlyShadowVertexOutput inputValue)
    {
        if (inputValue.castShadow == 0u)
        {
            discard_fragment();
        }
        WebglLoaderPlyShadowFrameBuffer frameBuffer;
        return frameBuffer;
    }
};

/** Draws ground, ASCII dolphins, and binary Lucy with r185 lighting, shadows, and fog. */
class WebglLoaderPlyLitPass final : public IRenderClass
{
public:
    /** Binds the unique Scene RenderSet and the two DSL-created shadow depth textures. */
    constructor(RenderSet<WebglLoaderPlySceneRenderSet> sceneSet [[Slot0]], BindGroup<WebglLoaderPlyLitResources> lightingResources [[Slot1]])
    {
...
private:
    ...
    WebglLoaderPlyLitVertexOutput vertex(WebglLoaderPlyVertex inputValue [[VertexInput0]], uint renderEntityID [[RenderEntityID]])
    {
        const WebglLoaderPlyObjectData objectData = sceneSet->objects->get(renderEntityID, 0u);
        const WebglLoaderPlyInstanceData instanceData = sceneSet->instances->get(renderEntityID, 0u);
        const WebglLoaderPlyRenderFlags flags = sceneSet->renderFlags->get(renderEntityID, 0u);
        ...
        WebglLoaderPlyLitVertexOutput outputValue;
        ...
        outputValue.fogRange = float2(objectData.cameraPositionAndFogNear.w, objectData.cameraForwardAndFogFar.w);
        outputValue.entityAndPackedPhase = uint2(renderEntityID, flags.values.x | (flags.values.z << 8u));
        return outputValue;
    }

    /** Evaluates r185 Phong/Standard light accumulation, PCF shadows, output transfer, and fog. */
    WebglLoaderPlyLitFrameBuffer fragment(WebglLoaderPlyLitVertexOutput inputValue)
    {
        const WebglLoaderPlyMaterialData materialData = sceneSet->materials->get(inputValue.entityAndPackedPhase.x, 0u);
        const uint materialKind = inputValue.entityAndPackedPhase.y & 0xffu;
        ...
        const float shadow0 = receiveShadow == 0u ? 1.0f : webglLoaderPlyDirectionalShadow(lightingResources, 0u, inputValue.shadowClip0, inputValue.position.xy * 0.5f);
        const float shadow1 = receiveShadow == 0u ? 1.0f : webglLoaderPlyDirectionalShadow(lightingResources, 1u, inputValue.shadowClip1, inputValue.position.xy * 0.5f);
        ...
        float3 linearColor = (directIrradiance0 + directIrradiance1 + hemisphereIrradiance) * materialData.baseColor.xyz * inversePi;
        if (materialKind == 0u)
        {
            linearColor += directIrradiance0 * webglLoaderPlyPhongSpecular(lightDirection0, viewDirection, normal, materialData.specularAndSurfaceParameter.xyz, materialData.specularAndSurfaceParameter.w) + directIrradiance1 * webglLoaderPlyPhongSpecular(lightDirection1, viewDirection, normal, materialData.specularAndSurfaceParameter.xyz, materialData.specularAndSurfaceParameter.w);
        }
        else
        {
            linearColor += directIrradiance0 * webglLoaderPlyPhysicalSpecular(lightDirection0, viewDirection, normal) + directIrradiance1 * webglLoaderPlyPhysicalSpecular(lightDirection1, viewDirection, normal);
        }

        float3 displayColor = float3(webglLoaderPlyLinearToSrgb(linearColor.x), webglLoaderPlyLinearToSrgb(linearColor.y), webglLoaderPlyLinearToSrgb(linearColor.z));
        const float fogDepth = dot(inputValue.worldPosition - inputValue.cameraPosition, inputValue.cameraForward);
        const float fogFactor = smoothstep(inputValue.fogRange.x, inputValue.fogRange.y, fogDepth);
        displayColor = lerp(displayColor, float3(0.44705883f, 0.39215687f, 0.35686275f), fogFactor);

        WebglLoaderPlyLitFrameBuffer frameBuffer;
        frameBuffer.color = half4(half3(displayColor), half(1.0f));
        return frameBuffer;
    }
};

/** Owns the unique PLY Scene RenderSet and all shadow, lighting, and present work. */
class Phase1LoaderPlyRenderSetRenderer final : public AbstractRenderer
{
    Device device;
    Swapchain swapchain;
    Queue graphicsQueue;
    [[Export]] RenderSet<WebglLoaderPlySceneRenderSet> sceneSet;
    Buffer<WebglLoaderPlyShadowInvocationData, BufferUsage<Uniform, CopyDst>> shadowInvocationBuffer0;
    Buffer<WebglLoaderPlyShadowInvocationData, BufferUsage<Uniform, CopyDst>> shadowInvocationBuffer1;
    BindGroup<WebglLoaderPlyShadowInvocationBindGroup> shadowInvocationResources0;
    BindGroup<WebglLoaderPlyShadowInvocationBindGroup> shadowInvocationResources1;
    BindGroup<WebglLoaderPlyLitResources> litResources;
    RenderClass<WebglLoaderPlyShadowDepthPass> shadowPass0;
    RenderClass<WebglLoaderPlyShadowDepthPass> shadowPass1;
    RenderClass<WebglLoaderPlyLitPass> litPass;
...
public:
    ...
    void init(Device inDevice, Swapchain inSwapchain) override
    {
        ...
        sceneSet = device->createRenderSet<WebglLoaderPlySceneRenderSet>();
        shadowInvocationBuffer0 = device->createBuffer("WebglLoaderPlyShadowInvocation0", 1u);
        shadowInvocationBuffer1 = device->createBuffer("WebglLoaderPlyShadowInvocation1", 1u);
        shadowInvocationResources0 = device->createBindGroup<WebglLoaderPlyShadowInvocationBindGroup>(shadowInvocationBuffer0);
        shadowInvocationResources1 = device->createBindGroup<WebglLoaderPlyShadowInvocationBindGroup>(shadowInvocationBuffer1);
        shadowPass0 = device->createRenderClass<WebglLoaderPlyShadowDepthPass>(sceneSet, shadowInvocationResources0);
        shadowPass1 = device->createRenderClass<WebglLoaderPlyShadowDepthPass>(sceneSet, shadowInvocationResources1);
        shadowDepth0 = device->createTexture("WebglLoaderPlyShadowDepth0", WebglLoaderPlyShadowMapSize, WebglLoaderPlyShadowMapSize, 1u);
        shadowDepth1 = device->createTexture("WebglLoaderPlyShadowDepth1", WebglLoaderPlyShadowMapSize, WebglLoaderPlyShadowMapSize, 1u);
    }
    ...
    void configureOutput(uint width, uint height)
    {
        ...
        outputColor = device->createTexture("WebglLoaderPlyOutputRGBA8", width, height, 1u);
        sceneDepth = device->createTexture("WebglLoaderPlySceneDepth32", width, height, 1u);
        litResources = device->createBindGroup<WebglLoaderPlyLitResources>(shadowDepth0->createView(), shadowDepth1->createView());
        litPass = device->createRenderClass<WebglLoaderPlyLitPass>(sceneSet, litResources);
    }
    ...
    {
        sceneSet->update();
        WebglLoaderPlyShadowInvocationData invocation0;
        invocation0.lightIndexAndReserved = uint4(0u, 0u, 0u, 0u);
        WebglLoaderPlyShadowInvocationData invocation1;
        invocation1.lightIndexAndReserved = uint4(1u, 0u, 0u, 0u);

        WebglLoaderPlyShadowFrameBuffer shadowFrameBuffer0;
        ...
        WebglLoaderPlyShadowFrameBuffer shadowFrameBuffer1;
        ...
        WebglLoaderPlyLitFrameBuffer litFrameBuffer;
        ...
        graphicsQueue->writeBuffer(BufferRange(shadowInvocationBuffer0), &invocation0, sizeof(invocation0))
            ...
            ->renderPass("WebglLoaderPlyShadow0", shadowFrameBuffer0, shadowPass0())
            ->renderPass("WebglLoaderPlyShadow1", shadowFrameBuffer1, shadowPass1())
            ->renderPass("WebglLoaderPlyMainLitFog", litFrameBuffer, litPass())
