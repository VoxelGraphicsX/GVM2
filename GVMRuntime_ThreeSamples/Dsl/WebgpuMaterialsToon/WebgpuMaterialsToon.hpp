#ifndef GVM_THREE_WEBGPUMATERIALSTOON_HPP
#define GVM_THREE_WEBGPUMATERIALSTOON_HPP

#include "UGL.h"

using namespace UGL;

static const uint WebgpuMaterialsToonTextureCapacity = 2u;
static const float WebgpuMaterialsToonGradientAtlasWidth = 27.0f;

/** Stores the union of position, normal, and barycentric edge attributes. */
struct WebgpuMaterialsToonVertex
{
...
struct WebgpuMaterialsToonObjectData
{
...
struct WebgpuMaterialsToonInstanceData
{
    float4 reserved;
};

/** Stores one material color and wireframe phase. */
struct WebgpuMaterialsToonMaterialData
{
    float4 baseColorAndFlags;
...
struct WebgpuMaterialsToonSamplerResources final : public IBindGroup
{
...
struct WebgpuMaterialsToonSceneRenderSet : public IRenderSet
{
    /** Declares packed geometry and per-entity transform/material components. */
    constructor(
        BufferComponent<WebgpuMaterialsToonVertex> vertices [[RenderSetVertexBuffer]],
        BufferComponent<uint> indices [[RenderSetIndexBuffer]],
        BufferComponent<WebgpuMaterialsToonObjectData> objects,
        BufferComponent<WebgpuMaterialsToonInstanceData> instances,
        BufferComponent<WebgpuMaterialsToonMaterialData> materials,
        (TextureComponent<half4, WebgpuMaterialsToonTextureCapacity> textures))
    {
    }
};

/** Carries transformed position, normal, barycentric coordinates, and entity id. */
struct WebgpuMaterialsToonVertexOutput
{
...
struct WebgpuMaterialsToonFrameBuffer final : public IFrameBuffer
{
...
float webgpuMaterialsToonLinearToSrgb(float value)
{
...
class WebgpuMaterialsToonMainPass final : public IRenderClass
{
public:
    /** Binds the Scene RenderSet and gradient sampler for the toon phase. */
    constructor(RenderSet<WebgpuMaterialsToonSceneRenderSet> sceneSet [[Slot0]],
                BindGroup<WebgpuMaterialsToonSamplerResources> samplerResources [[Slot1]])
    {
...
private:
    ...
    WebgpuMaterialsToonVertexOutput vertex(
        WebgpuMaterialsToonVertex inputValue [[VertexInput0]],
        uint renderEntityID [[RenderEntityID]],
        uint renderEntityInstanceID [[RenderEntityInstanceID]])
    {
        const WebgpuMaterialsToonObjectData objectData =
            sceneSet->objects->get(renderEntityID, 0u);
        const WebgpuMaterialsToonInstanceData instanceData =
            sceneSet->instances->get(renderEntityID, renderEntityInstanceID);
        const float4 localPosition = inputValue.position + float4(instanceData.reserved.xyz, 0.0f);
        WebgpuMaterialsToonVertexOutput outputValue;
        ...
        outputValue.viewNormal = localNormal;
    ...
    WebgpuMaterialsToonFrameBuffer fragment(
        WebgpuMaterialsToonVertexOutput inputValue)
    {
        const WebgpuMaterialsToonObjectData objectData =
            sceneSet->objects->get(inputValue.entityID, 0u);
        const WebgpuMaterialsToonMaterialData materialData =
            sceneSet->materials->get(inputValue.entityID, 0u);
        if (objectData.baseColorAndFlags.w > 0.5f)
        {
            WebgpuMaterialsToonFrameBuffer basicFrame;
        ...
        // r185's ToonLightingModel deliberately evaluates normalGeometry
        // (local-space, unnormalized) against PointLightNode's view-space
        // light direction. Preserve that observable coordinate-space behavior
        // instead of substituting the physically cleaner local-space vector.
        ...
        const float atlasCoordinate = (
            ...
            WebgpuMaterialsToonGradientAtlasWidth;
        ...
        const float3 srgb = float3(
            webgpuMaterialsToonLinearToSrgb(normalColor.x),
            webgpuMaterialsToonLinearToSrgb(normalColor.y),
            webgpuMaterialsToonLinearToSrgb(normalColor.z));
        WebgpuMaterialsToonFrameBuffer frameBuffer;
...
class WebgpuMaterialsToonOutlinePass final : public IRenderClass
{
public:
...
private:
    ...
    /** Emits the opaque black silhouette of the control sphere. */
    WebgpuMaterialsToonFrameBuffer fragment(
        WebgpuMaterialsToonVertexOutput inputValue)
    {
        const WebgpuMaterialsToonObjectData objectData =
...
class WebgpuMaterialsToonRenderer final : public AbstractRenderer
{
    Device device;
    Swapchain swapchain;
    Queue graphicsQueue;
    [[Export]] RenderSet<WebgpuMaterialsToonSceneRenderSet> sceneSet;
    Sampler gradientSampler;
    BindGroup<WebgpuMaterialsToonSamplerResources> samplerResources;
    RenderClass<WebgpuMaterialsToonMainPass> mainPass;
    RenderClass<WebgpuMaterialsToonOutlinePass> outlinePass;
...
public:
    ...
    void init(Device inDevice, Swapchain inSwapchain) override
    {
        ...
        sceneSet = device->createRenderSet<WebgpuMaterialsToonSceneRenderSet>();
        gradientSampler = device->createSampler({
            .label = "WebgpuMaterialsToonGradientSampler",
        ...
        samplerResources = device->createBindGroup<WebgpuMaterialsToonSamplerResources>(gradientSampler);
        mainPass = device->createRenderClass<WebgpuMaterialsToonMainPass>(sceneSet, samplerResources);
        outlinePass = device->createRenderClass<WebgpuMaterialsToonOutlinePass>(sceneSet);
    }
    ...
    void configureOutput(uint inWidth, uint inHeight)
    {
        width = inWidth;
        height = inHeight;
        outputColor = device->createTexture("WebgpuMaterialsToonColor", width, height, 1u);
        outputDepth = device->createTexture("WebgpuMaterialsToonDepth", width, height, 1u);
    }
    ...
    {
        sceneSet->update();
        WebgpuMaterialsToonFrameBuffer outlineFrame;
        ...
        WebgpuMaterialsToonFrameBuffer mainFrame;
        ...
        graphicsQueue
            // ToonOutlinePassNode renders the back-side shell first; the
            ...
            ->renderPass("WebgpuMaterialsToonOutline", outlineFrame, outlinePass())
            ->renderPass("WebgpuMaterialsToonMain", mainFrame, mainPass())
