#ifndef GVM_THREE_MISC_CONTROLS_TRACKBALL_HPP
#define GVM_THREE_MISC_CONTROLS_TRACKBALL_HPP
...
struct MiscControlsTrackballVertex
{
    float4 position [[Attribute0]];
};

/** Stores the camera, view transform, light directions, and fog density. */
struct MiscControlsTrackballObjectData
{
...
struct MiscControlsTrackballInstanceData
{
    float4 translation;
};

/** Stores the Phong diffuse, specular, ambient, and shininess values. */
struct MiscControlsTrackballMaterialData
{
...
/** Defines the unique one-entity RenderSet used by the trackball-control Scene. */
struct MiscControlsTrackballSceneRenderSet : public IRenderSet
{
    /** Declares the exact five-component Manifest schema. */
    constructor(
        BufferComponent<MiscControlsTrackballVertex> vertices [[RenderSetVertexBuffer]],
        BufferComponent<uint> indices [[RenderSetIndexBuffer]],
        BufferComponent<MiscControlsTrackballObjectData> objects,
        BufferComponent<MiscControlsTrackballInstanceData> instances,
        BufferComponent<MiscControlsTrackballMaterialData> materials)
    {
    }
};

/** Carries view-space geometry and the entity identity to flat Phong shading. */
struct MiscControlsTrackballVertexOutput
{
...
struct MiscControlsTrackballFrameBuffer final : public IFrameBuffer
{
...
float miscControlsTrackballLinearToSrgb(float value)
{
...
class MiscControlsTrackballSceneMainPass final : public IRenderClass
{
public:
    /** Configures Three's opaque front-face triangle and depth semantics. */
    constructor(RenderSet<MiscControlsTrackballSceneRenderSet> sceneSet [[Slot0]])
    {
...
private:
    ...
    MiscControlsTrackballVertexOutput vertex(
        MiscControlsTrackballVertex inputValue [[VertexInput0]],
        uint renderEntityID [[RenderEntityID]],
        uint renderEntityInstanceID [[RenderEntityInstanceID]])
    {
        const MiscControlsTrackballObjectData objectData =
            sceneSet->objects->get(renderEntityID, 0u);
        const MiscControlsTrackballInstanceData instanceData =
        ...
        MiscControlsTrackballVertexOutput outputValue;
    ...
    MiscControlsTrackballFrameBuffer fragment(MiscControlsTrackballVertexOutput inputValue)
    {
        const MiscControlsTrackballObjectData objectData =
            sceneSet->objects->get(inputValue.entityID, 0u);
        const MiscControlsTrackballMaterialData materialData =
        ...
        float3 outputColor = float3(
            miscControlsTrackballLinearToSrgb(linearColor.x),
            miscControlsTrackballLinearToSrgb(linearColor.y),
            miscControlsTrackballLinearToSrgb(linearColor.z));
        ...
        MiscControlsTrackballFrameBuffer frameBuffer;
...
class MiscControlsTrackballRenderer final : public AbstractRenderer
{
    Device device;
    Swapchain swapchain;
    Queue graphicsQueue;
    [[Export]] RenderSet<MiscControlsTrackballSceneRenderSet> sceneSet;
    RenderClass<MiscControlsTrackballSceneMainPass> scenePass;
...
public:
    ...
    void init(Device inDevice, Swapchain inSwapchain) override
    {
        ...
        sceneSet = device->createRenderSet<MiscControlsTrackballSceneRenderSet>();
        scenePass = device->createRenderClass<MiscControlsTrackballSceneMainPass>(sceneSet);
    }
    ...
    void configureOutput(uint width, uint height)
    {
        ...
        outputTexture = device->createTexture("MiscControlsTrackballRGBA8", width, height, 1u);
        depthTexture = device->createTexture("MiscControlsTrackballDepth32", width, height, 1u);
    }
    ...
    void render() override
    {
        ...
        graphicsQueue
            ->renderPass("MiscControlsTrackballScene", frameBuffer, scenePass())
