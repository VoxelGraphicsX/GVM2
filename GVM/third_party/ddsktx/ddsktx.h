...
DDSKTX_API void ddsktx_get_sub(const ddsktx_texture_info *tex, ddsktx_sub_data *buff, const void *file_data, int size, int array_idx, int slice_face_idx, int mip_idx);
...
#define stc__makefourcc(_a, _b, _c, _d) (((uint32_t)(_a) | ((uint32_t)(_b) << 8) | ((uint32_t)(_c) << 16) | ((uint32_t)(_d) << 24)))
...
#define DDSKTX__DDSCAPS2_CUBEMAP_ALLSIDES (0 | DDSKTX__DDSCAPS2_CUBEMAP_POSITIVEX | DDSKTX__DDSCAPS2_CUBEMAP_NEGATIVEX | DDSKTX__DDSCAPS2_CUBEMAP_POSITIVEY | DDSKTX__DDSCAPS2_CUBEMAP_NEGATIVEY | DDSKTX__DDSCAPS2_CUBEMAP_POSITIVEZ | DDSKTX__DDSCAPS2_CUBEMAP_NEGATIVEZ)
...
static const ddsktx__dds_translate_pixel_format k__translate_dds_pixel[] = {{8, DDSKTX__DDPF_LUMINANCE, {0x000000ff, 0x00000000, 0x00000000, 0x00000000}, DDSKTX_FORMAT_R8},
...
static const ddsktx__format_info k__formats_info[] = {{"BC1", false},    {"BC2", true},      {"BC3", true},       {"BC4", false},       {"BC5", false}, {"BC6H", false}, {"BC7", true},   {"ETC1", false},  {"ETC2", false}, {"ETC2A", true}, {"ETC2A1", true}, {"PTC12", false}, {"PTC14", false}, {"PTC12A", true}, {"PTC14A", true}, {"PTC22", true},   {"PTC24", true},  {"ATC", false},  {"ATCE", false},   {"ATCI", false},     {"ASTC4x4", true}, {"ASTC5x5", true}, {"ASTC6x6", false},
                                                      {"ASTC8x5", true}, {"ASTC8x6", false}, {"ASTC10x5", false}, {"<unknown>", false}, {"A8", true},   {"R8", false},   {"RGBA8", true}, {"RGBA8S", true}, {"RG16", false}, {"RGB8", false}, {"R16", false},   {"R32F", false},  {"R16F", false},  {"RG16F", false}, {"RG16S", false}, {"RGBA16F", true}, {"RGBA16", true}, {"BGRA8", true}, {"RGB10A2", true}, {"RG11B10F", false}, {"RG8", false},    {"RG8S", false}};
...
static bool ddsktx__parse_ktx(ddsktx_texture_info *tc, const void *file_data, int size, ddsktx_error *err)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        else if (k__translate_ktx_fmt[i].internal_fmt_srgb == header.internal_format)
        {
            format = (ddsktx_format)i;
            break;
        }
...
static bool ddsktx__parse_dds(ddsktx_texture_info *tc, const void *file_data, int size, ddsktx_error *err)
{
    ...
    if (ddsktx__read(&r, &header, sizeof(header)) < DDSKTX__DDS_HEADER_SIZE || header.size != DDSKTX__DDS_HEADER_SIZE)
    {
    ...
    if (DDSKTX__DDPF_FOURCC == (header.flags & DDSKTX__DDPF_FOURCC) && header.pixel_format.fourcc == DDSKTX__DDS_DX10)
    {
    ...
    if (dxgi_format == 0)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (f->bit_count == header.pixel_format.rgb_bit_count && f->flags == header.pixel_format.flags && f->bit_mask[0] == header.pixel_format.bit_mask[0] && f->bit_mask[1] == header.pixel_format.bit_mask[1] && f->bit_mask[2] == header.pixel_format.bit_mask[2] && f->bit_mask[3] == header.pixel_format.bit_mask[3])
                {
                    format = f->format;
                    break;
                }
...
void ddsktx_get_sub(const ddsktx_texture_info *tc, ddsktx_sub_data *sub_data, const void *file_data, int size, int array_idx, int slice_face_idx, int mip_idx)
{
    ddsktx_assert(tc);
