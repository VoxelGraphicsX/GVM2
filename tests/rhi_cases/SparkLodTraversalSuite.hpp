#ifndef GVM_TEST_RHI_SPARK_LOD_TRAVERSAL_SUITE_HPP
#define GVM_TEST_RHI_SPARK_LOD_TRAVERSAL_SUITE_HPP

#include "UGL.h"

using namespace UGL;

namespace SparkLodTraversalTest
{
    ...
    /**
     * Stores the unified Spark LoD cluster node layout used by CPU build code and GPU traversal.
     */
    ...
    /**
     * Stores immutable traversal constants for one Spark LoD cluster traversal dispatch sequence.
     */
    ...
    /**
     * Binds the Spark LoD cluster tree, frontier buffers, state counters, and selected cluster output.
     */
    ...
    /**
     * Computes the Spark LoD screen-space split error for one cluster without frustum culling.
     */
    ...
    /**
     * Computes the Spark-style radial pixel-scale priority used by deterministic node-test budget propagation.
     */
