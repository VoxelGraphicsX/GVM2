...
struct LayoutPayload { float4x2 value; };
...
constructor(UniformBuffer<LayoutPayload> data [[Binding0]], RWStructuredBuffer<uint> output [[Binding1]]) {}
};
...
void compute(uint3 threadID [[DispatchThreadID]]) { uint lane = threadID.x; bindings->output[lane] = uint(bindings->data->value[2][1]); }
};
