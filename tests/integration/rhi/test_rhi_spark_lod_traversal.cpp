...
namespace
{
    ...
    constexpr uint32_t kStateCount = SparkLodTraversalTest::StateCount;
    constexpr auto kCompletionTimeout = std::chrono::seconds(5);

    /**
     * Stores aggregate diagnostics produced by the synthetic Spark LoD cluster builder.
     */
    ...
    {
        std::vector<SparkLodTraversalTest::ClusterTreeNode> nodes;
        std::vector<uint32_t> levelCounts;
        ClusterBuildStats stats = {};
    };

    /**
     * Stores one Morton-sorted source splat reference for Spark LoD real scene cluster builds.
     */
    ...
    struct SourceSplatMortonKeyLess
    {
        bool operator()(const SourceSplatMortonKey &lhs, const SourceSplatMortonKey &rhs) const
        {
            if (lhs.key != rhs.key)
            {
    ...
    /**
     * Stores the CPU reference frontier cut for one Spark LoD traversal scenario.
     */
    ...
    /**
     * Describes one synthetic Spark LoD cluster traversal scenario.
     */
    struct Scenario
    {
    ...
    /**
     * Rounds a dispatch thread count up to a full Spark LoD traversal workgroup.
     */
    uint32_t roundUpToWorkGroup(uint32_t value)
    {
        const uint32_t workGroupSize = SparkLodTraversalTest::WorkGroupSize;
    ...
    /**
     * Builds a stable Morton-sorted source index array for Spark LoD leaf cluster generation.
     */
    std::vector<SourceSplatMortonKey> buildMortonSortedSourceKeys(const CpuGaussianScene &scene)
    {
        ...
        std::sort(keys.begin(), keys.end(), SourceSplatMortonKeyLess{});
        return keys;
    }

    /**
     * Splits Morton-sorted source splats into leaf cluster ranges near the Spark LoD target size.
     */
    std::vector<SourceSplatRange> buildLeafSourceRanges(uint32_t sourceSplatCount, uint32_t targetSplatsPerCluster)
    {
        ...
        {
            throw std::invalid_argument("Spark LoD real scene cluster build requires at least one source splat");
        }
        if (targetSplatsPerCluster == 0u || targetSplatsPerCluster > kMaxSourceSplatsPerLeafCluster)
        {
            throw std::invalid_argument("Spark LoD real scene target cluster size is outside the supported range");
        }
     ...
     */
    ClusterAggregate makeParentAggregate(const std::vector<SparkLodTraversalTest::ClusterTreeNode> &nodes,
                                         uint32_t childStart,
                                         uint32_t childCount)
    {
        ClusterAggregate aggregate = {};
    ...
    /**
     * Converts an aggregate payload and tree topology into the official Spark LoD ClusterTreeNode layout.
     */
    SparkLodTraversalTest::ClusterTreeNode makeClusterNodeFromAggregate(const ClusterAggregate &aggregate,
                                                                            uint32_t nodeIndex,
                                                                            uint32_t level,
                                                                            uint32_t childStart,
                                                                            uint32_t childCount,
    ...
    {
        SparkLodTraversalTest::ClusterTreeNode node = {};
        ...
        node.childInfo = uint4(childStart, childCount, level, childCount == 0u ? SparkLodTraversalTest::ClusterFlagLeaf : 0u);
        node.clusterInfo = uint4(aggregate.sourceSplatCount, nodeIndex, parentIndex, 0u);
        return node;
    }

    /**
     * Builds the formal Spark LoD ClusterTreeNode array from a sanitized CPU Gaussian scene.
     */
    SyntheticClusterTree buildSparkClusterTreeFromScene(const CpuGaussianScene &scene, uint32_t targetSplatsPerCluster, uint32_t parentFanout)
    {
        if (scene.empty())
        {
            throw std::invalid_argument("Spark LoD real scene cluster build requires a non-empty scene");
        }
        if (targetSplatsPerCluster == 0u || targetSplatsPerCluster > kMaxSourceSplatsPerLeafCluster)
        {
            throw std::invalid_argument("Spark LoD real scene target cluster size is outside the supported range");
        ...
        if (parentFanout == 0u)
        {
            throw std::invalid_argument("Spark LoD real scene cluster build requires a non-zero parent fanout");
        }
        if (scene.splats.size() > std::numeric_limits<uint32_t>::max())
        {
            throw std::invalid_argument("Spark LoD real scene cluster build currently supports at most uint32_t source splats");
        }
        ...
        for (uint32_t leafOrdinal = 0u; leafOrdinal < leafRanges.size(); ++leafOrdinal)
        {
            ...
            const uint32_t parentIndex = leafLevelIndex == 0u
                                             ? SparkLodTraversalTest::InvalidIndex
    ...
    /**
     * Computes the same screen-space split metric as the Spark LoD GPU node-test pass.
     */
    float cpuClusterScreenErrorPx(const SparkLodTraversalTest::ClusterTreeNode &cluster,
                                  const SparkLodTraversalTest::ClusterTraversalGlobals &globals)
    {
     ...
     */
    uint32_t cpuClusterSelectedEntryCost(const SparkLodTraversalTest::ClusterTreeNode &cluster)
    {
     ...
     */
    uint32_t cpuClusterBudgetMinEntryCost(const SparkLodTraversalTest::ClusterTreeNode &cluster)
    {
     ...
     */
    uint32_t cpuClusterBudgetFullEntryCost(const SparkLodTraversalTest::ClusterTreeNode &cluster)
    {
     ...
     */
    float cpuClusterBudgetPriority(const SparkLodTraversalTest::ClusterTreeNode &cluster,
                                   const SparkLodTraversalTest::ClusterTraversalGlobals &globals)
    {
        const float depth = std::max(cluster.boundsCenterRadius.z, globals.lod.z);
        const float focal = std::max(globals.projection.x, globals.projection.y);
    ...
    bool cpuClusterHasValidChildren(const SyntheticClusterTree &tree,
                                    const SparkLodTraversalTest::ClusterTreeNode &cluster)
    {
    ...
    uint32_t cpuChildEntryBudget(const SyntheticClusterTree &tree,
                                 const SparkLodTraversalTest::ClusterTraversalGlobals &globals,
                                 const SparkLodTraversalTest::ClusterTreeNode &parent,
                                 ...
                                 uint32_t targetChildNodeIndex)
    {
        ...
        uint32_t lastUnlockedNodeIndex = SparkLodTraversalTest::InvalidIndex;
        for (uint32_t unlockOrdinal = 0u; unlockOrdinal < parent.childInfo.y; ++unlockOrdinal)
        {
            float bestPriority = -1.0f;
            uint32_t bestNodeIndex = SparkLodTraversalTest::InvalidIndex;
            ...
            for (uint32_t childOrdinal = 0u; childOrdinal < parent.childInfo.y; ++childOrdinal)
            {
                ...
                if (childNodeIndex < clusterNodeCount)
                {
                    ...
                    if (unlockCost > 0u && unlockCost <= remainingBudget && belowLastUnlockedChild &&
                        (bestNodeIndex == SparkLodTraversalTest::InvalidIndex || priority > bestPriority || (priority == bestPriority && childNodeIndex < bestNodeIndex)))
                    {
            ...
            if (bestNodeIndex == SparkLodTraversalTest::InvalidIndex)
            {
                break;
            }
        ...
        if (unlockedChunkTotal > 0u && targetChunkCost > 0u && targetBudget < targetFullCost)
        {
            ...
            uint32_t replayLastUnlockedNodeIndex = SparkLodTraversalTest::InvalidIndex;
     ...
     */
    SparkLodTraversalTest::ClusterTraversalGlobals makeGlobals(uint32_t clusterNodeCount,
    ...
    {
        SparkLodTraversalTest::ClusterTraversalGlobals globals = {};
     ...
     */
    SparkLodTraversalTest::ClusterTreeNode makeSyntheticClusterNode(uint32_t nodeIndex,
                                                                        uint32_t level,
                                                                        uint32_t maxDepth,
                                                                        uint32_t childStart,
                                                                        uint32_t childCount,
                                                                        uint32_t parentIndex)
    {
        ...
        node.childInfo = uint4(childStart, childCount, level, isLeaf ? SparkLodTraversalTest::ClusterFlagLeaf : 0u);
    ...
    /**
     * Populates runtime-budget-independent subtree budget metadata for a synthetic Spark LoD tree.
     */
     ...
     */
    SyntheticClusterTree buildSyntheticSparkLodTree(uint32_t sourceSplatCount, uint32_t targetSplatsPerCluster, uint32_t parentFanout)
    {
        if (sourceSplatCount == 0u)
        {
            throw std::invalid_argument("Spark LoD synthetic cluster tree requires at least one source splat");
        }
        if (targetSplatsPerCluster == 0u || targetSplatsPerCluster > kMaxSourceSplatsPerLeafCluster)
        {
            throw std::invalid_argument("Spark LoD synthetic cluster tree target cluster size is outside the supported range");
        }
        if (parentFanout == 0u)
        {
            throw std::invalid_argument("Spark LoD synthetic cluster tree requires a non-zero parent fanout");
        }
    ...
    /**
     * Builds the exact CPU frontier cut expected from the Spark LoD GPU cluster traversal.
     */
    ...
    /**
     * Waits up to the fixed Spark LoD watchdog interval for the GPU completion flag.
     */
     ...
     */
    TEST(SparkLodBuilderTest, BuildsClusterTreeFromCpuGaussianScene)
    {
        const CpuGaussianScene scene = DebugSceneFactory::makeCalibrationScene(13u, 0.18f);
        const SyntheticClusterTree tree = buildSparkClusterTreeFromScene(
            scene,
        ...
        EXPECT_EQ(tree.nodes.front().clusterInfo.z, SparkLodTraversalTest::InvalidIndex);
        ...
        for (uint32_t clusterIndex = 0u; clusterIndex < tree.nodes.size(); ++clusterIndex)
        {
            ...
            if (clusterIndex != 0u)
            {
                ASSERT_NE(node.clusterInfo.z, SparkLodTraversalTest::InvalidIndex);
            ...
            if (childCount == 0u)
            {
                EXPECT_EQ(node.childInfo.w & SparkLodTraversalTest::ClusterFlagLeaf, SparkLodTraversalTest::ClusterFlagLeaf);
                continue;
            }
    ...
    /**
     * Builds Spark LoD ClusterTreeNode diagnostics from an optional real PLY scene path.
     */
    TEST(SparkLodBuilderTest, BuildsClusterTreeFromOptionalPlyScene)
    {
        const char *scenePath = std::getenv("GVM_TEST_SPARK_LOD_CLUSTER_PLY_PATH");
        if (scenePath == nullptr || scenePath[0] == '\0')
        {
            GTEST_SKIP() << "Set GVM_TEST_SPARK_LOD_CLUSTER_PLY_PATH to run the optional real PLY Spark LoD cluster builder diagnostic.";
        }
        ...
        std::fprintf(
            stderr,
            "[gvm-rhi-spark-lod-builder] path=%s source_splats=%u cluster_nodes=%u leaf_clusters=%u avg_leaf_splats=%.3f max_leaf_splats=%u depth=%u max_child_count=%u\n",
            scenePath,
    ...
    /**
     * Runs Spark LoD Cluster traversal scenarios against the active RHI backend.
     */
    class SparkLodTraversalRhiTest : public ::testing::Test
    {
    protected:
        /**
        ...
        void SetUp() override
        {
            ...
            {
                GTEST_SKIP() << "Spark LoD Cluster traversal test requires Metal or Vulkan backend, got " << backendName(backend);
            }
        ...
        /**
         * Builds, uploads, dispatches, and validates one Spark LoD cluster traversal scenario.
         */
        void runScenario(const Scenario &scenario)
        {
            ASSERT_GT(scenario.sourceSplatCount, 0u);
            SyntheticClusterTree tree = buildSyntheticSparkLodTree(
            ...
            auto activeClusterIndexBuffer = rawDevice->createBuffer({
                .label = "SparkLodActiveClusterIndex",
            ...
            auto completionBuffer = rawDevice->createBuffer({
                .label = "SparkLodTraversalCompletion",
            ...
            uploadQueue
                ...
                ->writeBuffer(GVM::RHI::BufferRange(clusterNodesBuffer, 0u, uint64_t(tree.nodes.size()) * sizeof(SparkLodTraversalTest::ClusterTreeNode)),
                              tree.nodes.data(),
                              uint64_t(tree.nodes.size()) * sizeof(SparkLodTraversalTest::ClusterTreeNode))
            ...
            auto traversalAToB = device->createBindGroup<SparkLodTraversalTest::ClusterTraversalBindGroup>(
            ...
            auto traversalBToA = device->createBindGroup<SparkLodTraversalTest::ClusterTraversalBindGroup>(
                GVM::RHI::BufferRange(globalsBuffer),
                GVM::RHI::BufferRange(clusterNodesBuffer),
                GVM::RHI::BufferRange(frontierBBuffer),
                GVM::RHI::BufferRange(frontierBudgetBBuffer),
                ...
                GVM::RHI::BufferRange(stateBuffer),
                GVM::RHI::BufferRange(activeClusterIndexBuffer));
            auto traversalStateBindGroup = device->createBindGroup<SparkLodTraversalTest::ClusterTraversalStateBindGroup>(
                GVM::RHI::BufferRange(globalsBuffer),
                GVM::RHI::BufferRange(stateBuffer));
            auto completionBindGroup = device->createBindGroup<SparkLodTraversalTest::CompletionBindGroup>(
            ...
            auto evaluateFrontierAToBPass = device->createComputeClass<SparkLodTraversalTest::EvaluateClusterFrontierPass>(traversalAToB);
            auto evaluateFrontierBToAPass = device->createComputeClass<SparkLodTraversalTest::EvaluateClusterFrontierPass>(traversalBToA);
            auto prepareNextFrontierPass = device->createComputeClass<SparkLodTraversalTest::PrepareNextFrontierPass>(traversalStateBindGroup);
            auto completionPass = device->createComputeClass<SparkLodTraversalTest::SignalCompletionPass>(completionBindGroup);
            ...
            const auto start = std::chrono::steady_clock::now();
            auto computePass = commandEncoder->beginComputePass({.label = "SparkLodTraversal"});
            ...
            ASSERT_TRUE(waitForCompletion(completionBuffer))
                << "GPU Spark LoD cluster traversal timed out after 5 seconds; scenario=" << scenario.label
            ...
            ASSERT_EQ(gpuState[SparkLodTraversalTest::StateOverflowCount], 0u);
            ASSERT_EQ(gpuState[SparkLodTraversalTest::StateCurrentFrontierCount], 0u);
            EXPECT_EQ(gpuState[SparkLodTraversalTest::StateProcessedClusterCount], cpu.processedCount);
            EXPECT_EQ(gpuState[SparkLodTraversalTest::StateSelectedClusterCount], static_cast<uint32_t>(cpu.selected.size()));
            EXPECT_EQ(gpuState[SparkLodTraversalTest::StateEstimatedEntryCount], cpu.estimatedEntryCount);
            EXPECT_LE(gpuState[SparkLodTraversalTest::StateEstimatedEntryCount], rootEntryBudget);
            EXPECT_EQ(gpuState[SparkLodTraversalTest::StateEvaluatedLevelCount], traversalLevelCount);
            ASSERT_LE(gpuState[SparkLodTraversalTest::StateSelectedClusterCount], outputCapacity);

            std::vector<uint32_t> gpuSelected(gpuState[SparkLodTraversalTest::StateSelectedClusterCount], 0u);
            ...
            std::fprintf(
                stderr,
                "[gvm-rhi-spark-lod] label=%s algorithm=spark-frontier-budgeted source_splats=%u cluster_nodes=%u leaf_clusters=%u avg_leaf_splats=%.3f max_leaf_splats=%u depth=%u threshold_px=%.3f root_entry_budget=%u estimated_entries=%u processed=%u selected=%zu max_dispatch_threads=%u total_dispatch_threads=%llu encoded_passes=%u evaluated_levels=%u gpu_ms=%.3f estimated_gpu_mb=%.2f\n",
                scenario.label,
                tree.stats.sourceSplatCount,
                tree.stats.clusterTreeNodeCount,
                tree.stats.leafClusterCount,
     ...
     */
    TEST_F(SparkLodTraversalRhiTest, SparkTraversalMatchesCpuReferenceForSmallTree)
    {
        runScenario({
            .label = "small-4k",
     ...
     */
    TEST_F(SparkLodTraversalRhiTest, SparkTraversalMatchesCpuReferenceForLargeTree)
    {
        runScenario({
            .label = "large-64k",
     ...
     */
    TEST_F(SparkLodTraversalRhiTest, SparkTraversalMatchesCpuReferenceForConfigurableTree)
    {
        const uint32_t sourceSplatCount = readEnvUint("GVM_TEST_SPARK_LOD_CLUSTER_MEDIUM_SOURCE_SPLATS", 1000000u);
        const float thresholdPx = readEnvFloat("GVM_TEST_SPARK_LOD_CLUSTER_MEDIUM_THRESHOLD_PX", 1.0f);
        const uint64_t budgetMb = readEnvUint64("GVM_TEST_SPARK_LOD_CLUSTER_MEDIUM_MEMORY_BUDGET_MB", 512ull);
     ...
     */
    TEST_F(SparkLodTraversalRhiTest, SparkTraversalMatchesCpuReferenceForPressureTree)
    {
        const uint32_t sourceSplatCount = readEnvUint("GVM_TEST_SPARK_LOD_CLUSTER_BICYCLE_SOURCE_SPLATS", 6131954u);
        const float thresholdPx = readEnvFloat("GVM_TEST_SPARK_LOD_CLUSTER_BICYCLE_THRESHOLD_PX", 1.0f);
        const uint32_t projectedEntryBudget = readEnvUint("GVM_TEST_SPARK_LOD_CLUSTER_BICYCLE_ENTRY_BUDGET", 1500000u);
        const uint64_t budgetMb = readEnvUint64("GVM_TEST_SPARK_LOD_CLUSTER_BICYCLE_MEMORY_BUDGET_MB", 1024ull);
    ...
    /**
     * Provides an opt-in 10M-source pressure case for Spark LoD Cluster traversal scaling checks.
     */
    TEST_F(SparkLodTraversalRhiTest, DISABLED_SparkTraversalMatchesCpuReferenceForTenMillionSourcePressure)
    {
        if (!readEnvBool("GVM_TEST_SPARK_LOD_CLUSTER_ENABLE_LARGE_PRESSURE", false))
        {
            GTEST_SKIP() << "Set GVM_TEST_SPARK_LOD_CLUSTER_ENABLE_LARGE_PRESSURE=1 and --gtest_also_run_disabled_tests to run the 10M-source Spark LoD pressure case.";
        }

        const uint32_t sourceSplatCount = readEnvUint("GVM_TEST_SPARK_LOD_CLUSTER_LARGE_SOURCE_SPLATS", 10000000u);
        const float thresholdPx = readEnvFloat("GVM_TEST_SPARK_LOD_CLUSTER_LARGE_THRESHOLD_PX", 1.0f);
        const uint64_t budgetMb = readEnvUint64("GVM_TEST_SPARK_LOD_CLUSTER_LARGE_MEMORY_BUDGET_MB", 1536ull);
